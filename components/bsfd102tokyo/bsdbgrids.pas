...
function TbsBookmarkList.Compare(const Item1, Item2: TBookmark): Integer;
begin
  with FGrid.Datalink.Datasource.Dataset do
    Result := CompareBookmarks(TBookmark(Item1), TBookmark(Item2));
end;

function TbsBookmarkList.CurrentRow: TBookmark;
begin
  if not FLinkActive then RaiseGridError(sDataSetClosed);
  Result := FGrid.Datalink.Datasource.Dataset.Bookmark;
end;
...
function TbsBookmarkList.Find(const Item: TBookmark; var Index: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
  if (Item = FCache) and (FCacheIndex >= 0) then
...
function TbsBookmarkList.GetItem(Index: Integer): TBookmark;
begin
  Result := FList[Index];
end;

function TbsBookmarkList.IndexOf(const Item: TBookmark): Integer;
begin
  if not Find(Item, Result) then
    Result := -1;
end;
...
procedure TbsBookmarkList.DeleteItem(Index: Integer);
var
  Temp: Pointer;
begin
...
procedure TbsBookmarkList.InsertItem(Index: Integer; Item: TBookmark);
begin
  if (Index < 0) or (Index > Count) then
    raise EListError.Create(' List Index Error');
  SetLength(FList, Count + 1);
...
procedure TbsBookmarkList.DataChanged(Sender: TObject);
begin
  FCache := nil;
  FCacheIndex := -1;
end;
...
procedure TbsSkinCustomDBGrid.DrawSkinGraphic(F: TField; Cnvs: TCanvas; R: TRect);
var
  DrawPict: TPicture;
  R1: TRect;
  CX, CY: Integer;
