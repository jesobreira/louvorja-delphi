...
type
  ...
  protected
    ...
    procedure SetPopupMenu(Value: TbsSkinPopupMenu); virtual;
  ...
  private
    ...
    FItemWidth: Integer;
    FBackButtonImageList: TCustomImageList;
    FBackButtonImageIndex,
    FBackButtonHotImageIndex: Integer;
    ...
    procedure SetBackButtonImageList(Value: TCustomImageList);
  ...
  published
    ...
    property SkinHint: TbsSkinHint
      read FSkinHint write FSkinHint;
    property BackButtonImageList: TCustomImageList
      read FBackButtonImageList write SetBackButtonImageList;
    property BackButtonImageIndex: Integer
      read FBackButtonImageIndex write FBackButtonImageIndex;
    property BackButtonHotImageIndex: Integer
      read FBackButtonHotImageIndex write FBackButtonHotImageIndex;
...
begin
  ...
  if (Ribbon <> nil) and Ribbon.HandleAllocated then Ribbon.Invalidate;
end;
...
begin
  ...
  if (Item <> nil) and Assigned(Item.OnClick)
  then
    Item.OnClick(Self);
end;
...
begin
  ...
  if (Operation = opRemove) and (AComponent = FAppButton)
  then
    begin
      FAppButton := nil;
    end;
  ...
  if (Operation = opRemove) and (FButtons <> nil) and
     (FButtons.Count > 0)
  then
    begin
      for i := 0 to FButtons.Count - 1 do
        if (FButtons[i].FPopupMenu <> nil) and
           (FButtons[i].FPopupMenu = AComponent)
        then
          FButtons[i].PopupMenu := nil;
    end;
end;
...
begin
  ...
  R := Rect(0, 0, 10, 10);
  with Buffer.Canvas, ButtonData do
  begin
    Font.Name := FontName;
    Font.Style := FontStyle;
    Font.Height := FontHeight;
  end;
  BSDrawSkinText(Buffer.Canvas, S, R, DT_CALCRECT);
  Result := RectWidth(R);
  Buffer.Free;
end;

function GetTextDefWidth(S: String): Integer;
var
  R: TRect;
  Buffer: TBitmap;
begin
...
begin
  ...
  Result := TbsBusinessSkinForm(BSF).IsModernSkin;
end;
...
var
  ...
  FModernSkin: Boolean;
begin
  if (Buttons <> nil) and (Buttons.Count > 0) and (ButtonsImageList <>  nil)
  then
    begin
      ...
      begin
        if Buttons[i].PopupMenu <> nil
        then w1 := w + 8 else w1 := w;
        if Buttons[i].Caption <> ''
        then
          if FIndex = -1
          then
            w1 := w1 + GetTextDefWidth(Buttons[i].Caption) + 6
          else
            w1 := w1 + GetTextWidth(Buttons[i].Caption) + 6;
        if w1 < h then w1 := h;
        Buttons[i].ItemRect := Rect(X - w1, 1, X, h + 1);
        Dec(X, w1);
      end;
    ...
    FRightOffset := 5;
  if (FMDIButtons <> nil) and (FMDIButtons.Count > 0)
  then
    begin
  ...
  FModernSkin := IsModernSkin;
  w := Width - FLeftOffset - FRightOffset;
  if FModernSkin then Dec(w, 3);
  w1 := 0;
  if FTabs <> nil then
  begin
    for i := 0 to Tabs.Count - 1 do
    if Tabs[i].Visible and (Tabs[i].Page <> nil) then
    begin
      R := Rect(0, 0, 0, 0);
...
procedure DrawButtonText(S: String; ItemRect: TRect; Active, Down, Enabled: Boolean);
var
  C: TColor;
  CIndex: Integer;
  ButtonData: TbsDataSkinButtonControl;
...
begin
  ...
  then
    ...
    then
      ...
      C := FSD.SkinColors.cBtnShadow;
  R := ItemRect;
  Inc(R.Left,  ButtonsImageList.Width + 6);
...
procedure DrawDefButtonText(S: String; ItemRect: TRect; Active, Down, Enabled: Boolean);
var
  C: TColor;
  R: TRect;
begin
...
procedure DrawButtonArrow(ItemRect: TRect; Active, Down, Enabled: Boolean);
var
  C: TColor;
  CIndex: Integer;
  ButtonData: TbsDataSkinButtonControl;
...
procedure DrawButtonDefArrow(ItemRect: TRect; Active, Down, Enabled: Boolean);
var
  C: TColor;
  R: TRect;
begin
...
begin
    ...
    begin
      ...
      then
        begin
          ...
          begin
            DrawButtonDefArrow(ItemRect, Active, (Active and Down) or (FPopupButton = Index), Enabled);
            Dec(X, 3);
          end;
          ...
          if Caption <> ''
          then
            DrawDefButtonText(Caption, ItemRect, Active, (Active and Down) or (FPopupButton = Index), Enabled); 
        end;
    end
  else
  with Buttons[Index] do
  ...
  then
    begin
      ...
      if PopupMenu <> nil then
      begin
        DrawButtonArrow(ItemRect, Active, (Active and Down) or (FPopupButton = Index), Enabled);
        Dec(X, 3);
      end;
      if Caption <> '' then X := ItemRect.Left + 3;
      if (ImageIndex < ButtonsImageList.Count) and (ImageIndex >= 0)
      ...
      if Caption <> ''
      then
        DrawButtonText(Caption, ItemRect, Active, (Active and Down) or (FPopupButton = Index), Enabled);
    end;
end;
...
begin
  ...
  begin
    ...
    FPopupMenu := TbsRibbonButtonItem(Source).PopupMenu;
    FCaption := TbsRibbonButtonItem(Source).Caption;
  end
  else
    inherited Assign(Source);
end;

procedure TbsRibbonButtonItem.SetCaption(Value: String);
begin
  if FCaption <> Value
  then
    begin
...
procedure TbsRibbonButtonItem.SetPopupMenu(Value: TbsSkinPopupMenu);
begin
   if Value <> FPopupMenu
  then
    begin
      if FPopupMenu <> nil
      then
        FPopupMenu.RemoveFreeNotification(TbsRibbonButtonItems(Collection).Ribbon);

      FPopupMenu := Value;

      if FPopupMenu <> nil then
        FPopupMenu.FreeNotification(TbsRibbonButtonItems(Collection).Ribbon);
      Changed(False);
    end;
end;
...
procedure TbsRibbonCustomPage.UpdateNC;
begin
...
procedure TbsRibbonCustomPage.AlignControls(AControl: TControl; var ARect: TRect);
begin
  inherited;
...
procedure TbsRibbonCustomPage.DoMouseEnter;
begin
  if (csDesigning in ComponentState) then Exit;
  FMouseIn := True;
  if CheckActivation
...
procedure TbsRibbonCustomPage.DoMouseLeave;
var
  P: TPoint;
begin
  if (csDesigning in ComponentState) then Exit;
...
procedure TbsAppMenuCustomPage.UpdateNC;
begin
  SetWindowPos(Handle, 0, Self.Left, Self.Top, Self.Width, Self.Height,
    SWP_FRAMECHANGED or SWP_DRAWFRAME or
    SWP_NOSIZE or SWP_NOMOVE or SWP_NOZORDER);
...
procedure TbsAppMenuCustomPage.AlignControls(AControl: TControl; var ARect: TRect);
begin
  inherited;
  if not FUpdatingScrollInfo then
  begin
...
procedure TbsAppMenuCustomPage.DoMouseEnter;
begin
  if (csDesigning in ComponentState) then Exit;
  FMouseIn := True;
  if CheckActivation
...
procedure TbsAppMenuCustomPage.DoMouseLeave;
var
  P: TPoint;
begin
  if (csDesigning in ComponentState) then Exit;
  ...
            B := False;
    ...
    WM_NCLBUTTONDOWN, WM_NCLBUTTONDBLCLK:
     ...
     begin
       ...
       P.X := TWMLButtonUp(Message).XPos;
       P.Y := TWMLButtonUp(Message).YPos;
       if (P.Y < 0) and Buttons[0].Visible and
           Buttons[0].MouseIn and (FCapturedButton = 1)
       then
         ButtonUp(0)
       else
       if (P.Y > ClientHeight) and Buttons[1].Visible and
          Buttons[1].MouseIn and (FCapturedButton = 2)
       then
         ButtonUp(1);
       FCapturedButton := 0;
...
begin
  ...
  FSkinHint := nil;
  FBackButtonImageList := nil;
  FBackButtonImageIndex := -1;
  FBackButtonHotImageIndex := -1;
  FItemImageList := nil;
...
procedure TbsAppMenu.SetBackButtonImageList(Value: TCustomImageList);
begin
  FBackButtonImageList := Value;
end;
...
begin
  inherited;
  if (Operation = opRemove) and (AComponent is TbsAppMenuPage) and (FItems <> nil) then
  begin
    I := GetPageIndex(TbsAppMenuPage(AComponent));
    if I <> -1 then
    begin
      Items[I].FPage := nil;
      Invalidate;
  ...
  if (Operation = opRemove) and (AComponent = FBackButtonImageList)
  then
    FBackButtonImageList := nil;
end;
...
begin
  ...
  then
    begin
      ...
      then
        begin
          if (FBackButtonImageList <> nil) and (FBackButtonImageIndex >= 0) and
             (FBackButtonImageIndex < FBackButtonImageList.Count) then
          begin
            X := FBackButtonImageList.Width;
            Y := FBackButtonImageList.Height;
            BackButtonOffset := Y + 5;
          end
          else
          begin
            BackButtonOffset := RectHeight(FData.BackButtonRect) + 5;
...
procedure TbsAppMenu.DrawAppMenuCaption(Cnvs: TCanvas; R, FR: TRect);
var
  Buffer: TBitmap;
  TempR: TRect;
begin
      ...
      then
        begin
          if (FBackButtonImageList <> nil) and (FBackButtonImageIndex >= 0) and
             (FBackButtonImageIndex < FBackButtonImageList.Count) then
          begin
           if FBackButtonActive and (FBackButtonHotImageIndex >= 0) and
             (FBackButtonHotImageIndex < FBackButtonImageList.Count) then
             FBackButtonImageList.Draw(Buffer.Canvas,
               FBackButtonRect.Left, FBackButtonRect.Top, FBackButtonHotImageIndex)
           else
             FBackButtonImageList.Draw(Buffer.Canvas,
               FBackButtonRect.Left, FBackButtonRect.Top, FBackButtonImageIndex)
          end
          else
          begin
...
procedure TbsAppMenuPageListBox.EnableScrollTimer(Value: Integer);
begin
...
procedure TbsAppMenuPageListBox.StopScrollTimer;
begin
  if FTimerMode <> 0 then
  begin
    FTimerMode := 0;
...
procedure TbsAppMenuPageListBox.WMTimer;
begin
  inherited;
  if Message.TimerID = 1 then
  begin
...
begin
  ...
  CIndex := SkinData.GetControlIndex('menupagelistbox');
  if CIndex = -1 then
