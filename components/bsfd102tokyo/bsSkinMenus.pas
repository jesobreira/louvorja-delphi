...
uses
  ...
  bsSkinHint{$IFDEF TNTUNICODE}, TntMenus {$ENDIF};
...
type
  ...
  {$IFDEF TNTUNICODE}
  TbsSkinPopupMenu = class(TTntPopupMenu)
  {$ELSE}
...
procedure TbsSkinMenuItem.ResizeDraw(Cnvs: TCanvas);
var
  GX, GY: Integer;
  DrawGlyph: Boolean; 
  kf: Double;
...
begin
  ...
  then
    begin
      ...
      then
        begin
          LData := TbsDataSkinLabelControl(Parent.SD.CtrlList[I]);
          ...
          //
          R2 := R1;
          BSDrawSkinText(Buffer.Canvas, Caption, R2, DT_CALCRECT);
          R1.Top := R1.Top + RectHeight(R1) div 2 - RectHeight(R2) div 2;
          R1.Bottom := R1.Top + RectHeight(R2);
          BSDrawSkinText(Buffer.Canvas, Caption, R1,
            Parent.ParentMenu.FForm.DrawTextBiDiModeFlags(DT_CENTER or DT_VCENTER));
          //
...
begin
  ...
  begin
    ...
    BSDrawSkinText(B.Canvas, S, R, DT_CALCRECT);
    OffsetRect(R, 0, TR.Top + RectHeight(TR) div 2 - R.Bottom div 2);
    Inc(R.Right, 2);
    //
...
procedure TbsSkinPopupWindow.DrawScrollArea(Cnvs: TCanvas; R: TRect);
var
...
begin
  ...
  MI := TbsDataSkinMenuItem(SD.ObjectList[I]);
  ...
  case CharCode of
    ...
    VK_LEFT:
      begin
        ...
        then
          begin
            PW := TbsSkinPopupWindow(ParentMenu.FPopupList.Items[ParentMenu.FPopupList.Count - 2]);
            if PW.ActiveItem <> -1
            then
...
function TbsSkinPopupWindow.GetItemHeight: Integer;
begin
...
begin
  ...
  then
    CreateSkinImageBS(PW.LTPoint, PW.RTPoint, PW.LBPoint, PW.RBPoint,
      ...
      Width, Height, ADrawClient, PW.LeftStretch, PW.TopStretch,
  ...
  then
   ...
   then
     begin
       ...
       ItemList.Add(TbsSkinMenuItem.Create(Self, TMenuItem(miL.Items[i]), DSMI));
     end;
...
begin
  ...
  begin
    ...
    Y := TbsSkinMenuItem(ItemList.Items[i]).ObjectRect.Bottom;
  end;
...
function TbsSkinMenu.GetWorkArea(P: TPoint): TRect;
begin
...
begin
  ...
  then
    begin
      BSF := TbsBusinessSkinForm(FSkinComponent);
    end
  else
...
procedure TbsSkinImagesMenuPopupWindow.FindUp;
var
  I: Integer;
begin
...
procedure TbsSkinImagesMenuPopupWindow.FindDown;
var
  I: Integer;
begin
  if Self.MouseInItem = -1 then
...
procedure TbsSkinImagesMenuPopupWindow.FindRight;
var
  I: Integer;
begin
  if Self.MouseInItem = -1 then
...
procedure TbsSkinImagesMenuPopupWindow.FindLeft;
var
  I: Integer;
begin
  if Self.MouseInItem = -1 then
...
procedure TbsSkinImagesMenuPopupWindow.ProcessKey(KeyCode: Integer);
begin
  case KeyCode of
...
function TbsSkinImagesMenuPopupWindow.GetLabelDataControl: TbsDataSkinLabelControl;
var
  I: Integer;
begin
  ...
  if I <> -1
  then
    Result := TbsDataSkinLabelControl(ImagesMenu.SkinData.CtrlList[I])
  else
    Result := nil;
end;
...
var
  ...
  R: TRect;
  LabelData: TbsDataSkinLabelControl;
begin
...
begin
  ...
  then
    ...
    begin
      ...
      Frame3D(Buffer.Canvas, SknR, BS_BTNFRAMECOLOR, BS_BTNFRAMECOLOR, 1);
      if ASelected
      then
        Buffer.Canvas.Brush.Color := BS_BTNDOWNCOLOR
      else
        Buffer.Canvas.Brush.Color := BS_BTNACTIVECOLOR;
