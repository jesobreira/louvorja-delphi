...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls, Forms,
     ...
     StdCtrls, ExtCtrls, bsSkinShellCtrls, bsColorCtrls, ImgList, bsPngImageList;
...
type
  ...
  public
    ...
    FontColorBox: TbsSkinTextColorButton;
    FontSizeEdit: TbsSkinFontSizeComboBox;
    ...
    constructor CreateEx(AOwner: TComponent; ACtrlSkinData: TbsSkinData; AShowScript: Boolean;
  ...
  public
    FCSkinList: TbsCompressedSkinList;
    FCSkinIndex: Integer;
    BSF: TbsBusinessSkinForm;
    ...
    SkinList: TList;
    constructor CreateEx2(AOwner: TComponent;  ASkinList: TbsCompressedSkinList;
                          ASkinIndex: Integer;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function Execute(DefaultCompressedSkin: TbsCompressedStoredSkin): Boolean; overload;
  ...
  published
    property SkinList: TbsCompressedSkinList read FSkinList write FSkinList;
    property AlphaBlend: Boolean read FAlphaBlend write FAlphaBlend;
    property AlphaBlendValue: Byte read FAlphaBlendValue write FAlphaBlendValue;
    property AlphaBlendAnimation: Boolean
      read FAlphaBlendAnimation write FAlphaBlendAnimation;
  ...
  public
    ...
    constructor CreateEx(AOwner: TComponent; ASkinsFolder, ADefaultSkinFolder, AIniFileName: String;
                         ACtrlSkinData: TbsSkinData; ACompressed: Boolean);
    destructor Destroy; override;
    procedure SLBOnChange(Sender: TObject);
    procedure SLBOnDblClick(Sender: TObject);
  end;
...
begin
  ...
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_BOLD_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_ITALIC_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_UNDERLINE_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_STRIKEOUT_PNG');
...
var
  FW, FH: Integer;
begin
  FDlgFrm := TbsFontDlgForm.CreateEx(Application, CtrlSkinData,  FShowScript, FButtonsImageList);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
...
constructor TbsSelectSkinDlgForm.CreateEx2;
var
  I, Idx: Integer;
  S: String;
  ResStrData: TbsResourceStrData;
...
function TbsSelectSkinDialog.Execute(SkinIndex: Integer): Boolean;
var
  FW, FH, CIndex: Integer;
  Path: String;
begin
  if FSkinList = nil then Exit;
  FDlgFrm := TbsSelectSkinDlgForm.CreateEx2(Application, FSkinList, SkinIndex, CtrlSkinData);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
...
function TbsSelectSkinDialog.Execute(DefaultCompressedSkin: TbsCompressedStoredSkin): Boolean;
var
  FW, FH, CIndex: Integer;
  Path: String;
begin
...
function TbsSelectSkinsFromFoldersDialog.Execute(ASkinsFolder, ADefaultSkinFile, ASkinExt: String): Boolean;
var
  FW, FH: Integer;
begin
  FDlgFrm := TbsSelectSkinsFromFoldersDlgForm.CreateEx(Application, ASkinsFolder, ADefaultSkinFile, ASkinExt, CtrlSkinData, True);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
...
function TbsSelectSkinsFromFoldersDialog.Execute2(ASkinsFolder, ADefaultSkinFolder, AIniFileName: String): Boolean;
var
  FW, FH: Integer;
begin
  FDlgFrm := TbsSelectSkinsFromFoldersDlgForm.CreateEx(Application, ASkinsFolder, ADefaultSkinFolder, AIniFileName, CtrlSkinData, False);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
...
procedure TbsSkinFindDialog.CloseDialog;
begin
...
procedure TbsSkinReplaceDialog.CloseDialog;
begin
  if Form <> nil then Form.Close;
end;
