...
{ TbsSkinCalculator }
  ...
  private
    ...
    FDisplayLabelDefaultHeight: Integer;
  ...
  published
    ...
    property DisplayLabelDefaultHeight: Integer
      read FDisplayLabelDefaultHeight write FDisplayLabelDefaultHeight;
...
{ TbsCalculatorForm }
  ...
  private
    ...
    FAlphaBlendAnimation: Boolean;
    FAlphaBlendValue: Byte;
    FCalcUseSkinFont: Boolean;
    FCalcDisplayLabelDefaultHeight: Integer;
  ...
  published
    ...
    property CalcUseSkinFont: Boolean
     read FCalcUseSkinFont write FCalcUseSkinFont;
    property CalcDisplayLabelDefaultHeight: Integer
      read FCalcDisplayLabelDefaultHeight write FCalcDisplayLabelDefaultHeight;
...
begin
  ...
  try
    ...
    else if Kind = cbDcm then Caption := {$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator
...
begin
  ...
  if Key in [{$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator, '.', ','] then begin
    CheckFirst;
    if Pos({$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator, FText) = 0 then
      SetText(FText + {$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator);
    Exit;
  end;
  case Key of
    'R':
...
var
  ...
  BtnTag: Longint;
begin
  if Key in [{$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator, '.', ','] then Key := '.'
...
begin
  case TCalcButton(Sender).Kind of
    ...
    cbSgn: CalcKey('_');
    cbDcm: CalcKey({$IFDEF VER240_UP}FormatSettings.{$ENDIF}DecimalSeparator);
    cbDiv: CalcKey('/');
    cbMul: CalcKey('*');
    cbSub: CalcKey('-');
    cbAdd: CalcKey('+');
  ...
  if Clipboard.HasFormat(CF_TEXT) then
    try
      SetDisplay(StrToFloat(Trim(ReplaceStr(Clipboard.AsText,
        {$IFDEF VER240_UP}FormatSettings.{$ENDIF}CurrencyString, ''))));
    except
      SetText('0');
    end;
end;
...
begin
  ...
  then
    ...
    FCalc.Caption := Self.Title;
  with FCalc do
  try
    FCalcPanel.SkinData := Self.CtrlSkinData;
    FDisplayLabel.DefaultFont := FDefaultFont;
    FDisplayLabel.DefaultHeight := FDisplayLabelDefaultHeight;
    FDisplayLabel.UseSkinFont := Self.UseSkinFont;
    FDisplayLabel.UseSkinSize := Self.UseSkinFont;
    ...
    FCalcPanel.Height := FCalcPanel.Height + BtnOffset;
...
begin
  ...
  FValue := 0;
  FIncrement := 1;
  FCalcUseSkinFont := True;
  FCalcDisplayLabelDefaultHeight := 20;
  FDecimal := 2;
  StopCheck := True;
  Text := '0';
  StopCheck := False;
...
function TbsSkinCalcEdit.GetCalcFont;
begin
  Result := FCalc.DefaultFont;
end;

procedure TbsSkinCalcEdit.SetCalcFont;
begin
  FCalc.DefaultFont.Assign(Value);
end;
...
begin
 with FCalc do
  begin
    ...
    //
    FDisplayLabel.DefaultFont.Assign(FCalc.DefaultFont);
    FDisplayLabel.DefaultHeight := FCalcDisplayLabelDefaultHeight;
    FDisplayLabel.UseSkinFont := Self.FCalcUseSkinFont;
    FDisplayLabel.UseSkinSize := Self.FCalcUseSkinFont;
    ...
    with TbsSkinSpeedButton(FCalcPanel.Controls[i]) do
    begin
      DefaultFont.Assign(FCalc.DefaultFont);
      UseSkinFont := Self.CalcUseSkinFont;
      DefaultHeight := 25;
...
function TbsSkinCalcCurrencyEdit.GetCalcFont;
begin
  Result := FCalc.DefaultFont;
end;

procedure TbsSkinCalcCurrencyEdit.SetCalcFont;
begin
  FCalc.DefaultFont.Assign(Value);
end;
