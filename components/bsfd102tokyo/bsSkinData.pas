...
type
 ...
 public
  ...
  TbsSystemColorHook = (bsschHighLight, bsschWindow);
  TbsSystemColorHooks = set of TbsSystemColorHook;
  ...
  protected
    ...
    FSystemColorHooks: TbsSystemColorHooks;
    //
  ...
  published
    ...
    property SystemColorHooks: TbsSystemColorHooks
...
begin
  ...
  begin
    case Color of
      COLOR_WINDOW:
        if bsschWindow in SysSkinData.SystemColorHooks then
          Color := ColortoRGB(SysSkinData.SkinColors.cWindow);
      COLOR_WINDOWTEXT:
        if bsschWindow in SysSkinData.SystemColorHooks then
          Color := ColortoRGB(SysSkinData.SkinColors.cWindowText);
      COLOR_HIGHLIGHT:
        if bsschHighLight in SysSkinData.SystemColorHooks then
...
begin
  ...
  begin
    if (nIndex = COLOR_WINDOW) and (bsschWindow in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cWindow)
    else
    if (nIndex = COLOR_WINDOWTEXT) and (bsschWindow in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cWindowText)
    else
    if (nIndex = COLOR_HIGHLIGHT) and (bsschHighLight in SysSkinData.SystemColorHooks)  then
      Result := ColortoRGB(SysSkinData.SkinColors.cHighLight)
    else
    if (nIndex = COLOR_HIGHLIGHTTEXT) and (bsschHighLight in SysSkinData.SystemColorHooks)  then
...
begin
  ...
  FormCaptionRect := ReadRect(IniFile, IDName, 'formcaptionrect');
end;
...
begin
  ...
  WriteRect(IniFile, IDName, 'formcaptionrect', FormCaptionRect);
end;
...
begin
  ...
  OfficeRect := ReadRect(IniFile, IDName, 'officerect');
  OfficeActiveRect := ReadRect(IniFile, IDName, 'officeactiverect');
  OfficeDownRect := ReadRect(IniFile, IDName, 'officedownrect');
  OfficeInActiveRect := ReadRect(IniFile, IDName, 'officeinactiverect');

  OfficeRestoreRect := ReadRect(IniFile, IDName, 'officerestorerect');
  OfficeRestoreActiveRect := ReadRect(IniFile, IDName, 'officerestoreactiverect');
  OfficeRestoreDownRect := ReadRect(IniFile, IDName, 'officerestoredownrect');
  OfficeRestoreInActiveRect := ReadRect(IniFile, IDName, 'officerestoreinactiverect');
...
begin
  ...
  WriteRect(IniFile, IDName, 'officerect', OfficeRect);
  WriteRect(IniFile, IDName, 'officeactiverect', OfficeActiveRect);
  WriteRect(IniFile, IDName, 'officedownrect', OfficeDownRect);
  WriteRect(IniFile, IDName, 'officeinactiverect', OfficeInActiveRect);

  WriteRect(IniFile, IDName, 'officerestorerect', OfficeRestoreRect);
  WriteRect(IniFile, IDName, 'officerestoreinactiverect', OfficeRestoreInActiveRect);
  WriteRect(IniFile, IDName, 'officerestoreactiverect', OfficeRestoreActiveRect);
  WriteRect(IniFile, IDName, 'officerestoredownrect', OfficeRestoreDownRect);
...
begin
  ...
  //
  WriteRect(IniFile, IDName, 'officerect', DividerRect);
  WriteRect(IniFile, IDName, 'officebgrect', OfficeBGRect);
  IniFile.WriteInteger(IDName, 'officefontcolor', OfficeFontColor);
  IniFile.WriteInteger(IDName, 'officeactivefontcolor', OfficeActiveFontColor);
end;
...
begin
  ...
  //
  OfficeBGRect := ReadRect(IniFile, IDName, 'officebgrect');
  OfficeFontColor:= IniFile.ReadInteger(IDName, 'officefontcolor', 0);
  OfficeActiveFontColor := IniFile.ReadInteger(IDName, 'officeactivefontcolor', 0);
end;
...
begin
  ...
  FSystemColorHooks := [bsschHighLight];
end;
...
procedure TbsSkinData.SetChangeSystemColors(Value: Boolean);
begin
