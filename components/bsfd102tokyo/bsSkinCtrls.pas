...
type
  ...
  TbsDrawItemImageEvent = procedure(AItem: TListItem;
  ...
  protected
    ...
    FOnDrawItemImage: TbsDrawItemImageEvent;
    //
  ...
  public
    ...
    property OnDrawSkinSubItem: TbsDrawSkinSubItemEvent
     ...
     property OnDrawItemImage: TbsDrawItemImageEvent read
...
var
  ...
  X, Y: Integer;
begin
  if FTransparent and not ADown and not AActive and not IsNullRect(MenuMarkerFlatRect)
  then
  begin
    SR := MenuMarkerFlatRect;
  end
  else
...
begin
  ...
  then
     begin
       if (MouseInControl <> FMouseIn) and 
         (FIndex <> -1) and EnableAnimation and not IsNullRect(AnimateSkinRect) then 
         StartAnimate(False);
       ReDrawControl;
     end;
...
procedure TbsSkinToolBar.AlignControls(AControl: TControl; var ARect: TRect);
begin
  inherited;
...
procedure TbsSkinToolBar.UpdateNC;
begin
...
procedure TbsSkinToolBar.DoMouseEnter;
begin
  if (csDesigning in ComponentState) then Exit;
  FMouseIn := True;
  if CheckActivation
...
procedure TbsSkinToolBar.DoMouseLeave;
var
  P: TPoint;
begin
...
procedure TbsSkinCustomListView.NewDefaultDrawItem(Sender: TCustomListView; Item: TListItem;
  State: TCustomDrawState);
var
...
procedure TbsSkinCustomListView.NewDrawItem(Sender: TCustomListView; Item: TListItem;
   State: TCustomDrawState);
var
  SelectBounds, TextBounds, IconBounds, ItemBounds, R, CR, R1: TRect;
  B, Buffer: TBitmap;
...
procedure TbsSkinCustomListView.NewAdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
...
procedure TbsSkinCustomListView.SetAlternateRow(Value: Boolean);
begin
...
procedure TbsSkinScrollPanel.AlignControls(AControl: TControl; var ARect: TRect);
begin
  inherited;
  if not FUpdatingScrollInfo then
    GetScrollInfo;
...
procedure TbsSkinScrollPanel.UpdateNC;
begin
  SetWindowPos(Handle, 0, Self.Left, Self.Top, Self.Width, Self.Height,
    SWP_FRAMECHANGED or SWP_DRAWFRAME or
    SWP_NOSIZE or SWP_NOMOVE or SWP_NOZORDER);
...
procedure TbsSkinScrollPanel.DoMouseEnter;
begin
  if (csDesigning in ComponentState) then Exit;
  FMouseIn := True;
  if CheckActivation
...
procedure TbsSkinScrollPanel.DoMouseLeave;
var
  P: TPoint;
begin
  if (csDesigning in ComponentState) then Exit;
