...
uses
  ...
  Forms, bsEffects, ExtCtrls, CommCtrl,
  bsPngImageList, bsPngImage{$IFDEF VER200}, PngImage {$ENDIF};

const
  maxi = 10000;

  // colors for default draw
  BS_BTNFRAMECOLOR = 12020539;
  BS_BTNACTIVECOLOR = 15979713;
  BS_BTNDOWNCOLOR = 14990498;
  BS_GAUGEFRAME = 1290752;
...
{ TbsFBitmap }
  ...
  TbsLayerWindow = class(TCustomControl)
  protected
    procedure WMMouseActivate(var Message: TMessage); message WM_MOUSEACTIVATE;
    procedure CreateParams(var Params: TCreateParams); override;
  end;

  TbsLayerManager = class(TComponent)
  protected
    LayerWindow: TbsLayerWindow;
    Bmp: TbsBitMap;
    Timer: TTimer;
...
procedure DrawDownArrowImage(Cnvs: TCanvas; R: TRect; Color: TColor);
procedure DrawArrowImage2(Cnvs: TCanvas; R: TRect; Color: TColor; Code: Integer);
procedure DrawArrowImage3(Cnvs: TCanvas; R: TRect; Color: TColor; Code: Integer);
procedure DrawRibbonArrowImage(Cnvs: TCanvas; R: TRect; Color: TColor);
...
procedure BSDrawText5(Cnvs: TCanvas; S: String; R: TRect);
procedure BSDrawTextCenterMultiLine(Cnvs: TCanvas; S: String; R: TRect; AModify: Boolean);
procedure BSDrawTextCenter(Cnvs: TCanvas; S: String; R: TRect);
procedure BSDrawTextAlignment(Cnvs: TCanvas; S: String; R: TRect; AAlignment: TAlignment);
...
procedure CreateAlphaLayer(ABmp: TBitmap; AMaskBmp: TBitMap;
                           AlphaBmp: TbsBitmap);
procedure CreateAlphaLayered2(AlphaBmp: TbsBitmap; AAlphaBlendValue: Integer;
...
var
  ...
  procedure DrawSkinVertLine(C: TCanvas; X, Y1, Y2: Integer; Color: TColor);
  procedure DrawSkinHorzLine(C: TCanvas; X1, X2, Y: Integer; Color: TColor);

  procedure MakeCopyFromPng(B :TbsBitMap; P: TbsPngImage);
  procedure MakeBlurBlank(B :TbsBitMap; P: TbsPngImage; BlurSize: Integer);
  procedure MakeBlurBlank2(B :TbsBitMap; SB: TBitmap; BlurSize: Integer);
  procedure MakeBlurBlank3(B :TbsBitMap; IL: TCustomImageList; Index: Integer; BlurSize: Integer);
  //
...
var
  ...
  SB: TBitMap;
begin
  if IL is TbsPngImageList
  then
    begin
      MakeBlurBlank(B, TbsPngImageList(IL).PngImages.Items[Index].PngImage, BlurSize);
      Exit;
    end
  else
    begin
...
begin
  ...
  if C.Pixels[X, Y1 - 1] = Color then B1 := False else B1 := True;
  for i := Y1 to Y2 do
  begin
    if B1 then C.Pixels[X, i] := Color;
    B1 := not B1;
  end;
end;
...
var
  ...
  X: Integer;
begin
  if C.Pixels[X1, Y] <> Color then B1 := True else B1 := False;
  if X1 > X2
  then
    begin
      for i := X1 downto X2 do
      begin
        if B1 then C.Pixels[i, Y] := Color;
        B1 := not B1;
      end;
    end
  else
...
procedure TbsLayerWindow.CreateParams(var Params: TCreateParams);
begin
...
procedure TbsLayerWindow.WMMouseActivate(var Message: TMessage);
begin
  Message.Result := MA_NOACTIVATE;
end;

constructor TbsLayerManager.Create(AOwner: TComponent);
begin
...
destructor TbsLayerManager.Destroy;
begin
  Timer.Free;
...
procedure TbsLayerManager.Update;
begin
...
procedure TbsLayerManager.Show;
const
  ULW_ALPHA = $00000002;
  WS_EX_LAYERED = $80000;
var
...
begin
  ...
  IsVisible := True;
  LayerWindow := TbsLayerWindow.Create(Self);
...
procedure TbsLayerManager.StartTimer;
begin
  BeginAlphaBlendValue := 0;
  Timer.Enabled := True;
end;

procedure TbsLayerManager.StopTimer;
begin
  Timer.Enabled := False;
end;

procedure TbsLayerManager.ExecTimer(Sender: TObject);
begin
...
procedure TbsLayerManager.Hide;
begin
  if not CheckW2KWXP then Exit;
  StopTimer;  
    ...
    begin
      ...
      for i := 0 to B.Width - 1 do
        for j := 0 to B.Height - 1 do
          begin
            PbsColorRec(B.PixelPtr[i, j]).Color := FromRGB(FGlowColor) and
...
begin
  R := Rect(0, 0, 0, 0);
  BSDrawSkinText(C, Str, R, DT_CALCRECT or DT_NOPREFIX);
  Result := RectWidth(R);
end;
...
begin
  ...
  with Cnvs do
  begin
    Cnvs.Pen.Color := Color;
    Cnvs.Pixels[X, Y] := Color;
    Cnvs.Pixels[X + 1, Y + 1] := Color;
    Cnvs.Pixels[X + 2, Y + 2] := Color;
    MoveTo(X + 3, Y);
    LineTo(X + 3, Y + 3);
    LineTo(X, Y + 3);
    LineTo(X + 3, Y);
...
begin
  ...
  for i := 0 to B.Width - 1 do
    for j := 0 to B.Height - 1 do
    begin
      PbsColorRec(B.PixelPtr[i, j]).Color := FromRGB(ShadowColor) and
        not $FF000000 or (PbsColorRec(B.PixelPtr[i, j]).R shl 24);
    end;
  B.AlphaBlend := true;
...
begin
  ...
  //
  OffsetRect(R, -1, -1);
  BSDrawSkinText(B.Canvas, Text, R, Flags);
  OffsetRect(R, -1, -1);
  BSDrawSkinText(B.Canvas, Text, R, Flags);
  OffsetRect(R, 2, 2);
  //
  ...
  for i := 0 to B.Width - 1 do
    for j := 0 to B.Height - 1 do
    begin
      PbsColorRec(B.PixelPtr[i, j]).Color := FromRGB(ShadowColor) and not $FF000000 or (PbsColorRec(B.PixelPtr[i, j]).R shl 24);
    end;
  B.AlphaBlend := true;
  ...
  if R.Bottom - R.Top > 0 then
    ...
    begin
      ...
      begin
        ...
        PbsColorRec(B.PixelPtr[i, j]).Color := FromRGB(ReflectionColor) and
        not $FF000000 or
        ((Trunc(PbsColorRec(B.PixelPtr[i, j]).R * Transparency) shl 24));
      end;
    end;
...
begin
  ...
  B.Free;
  Canvas.Brush.Style := bsClear;
  BSDrawSkinText(Canvas, Text, ARect, Flags);
end;
...
begin
  ...
  R1 := R;
  OffsetRect(R1, -1, 0);
  BSDrawSkinText(B.Canvas, Text, R1, Flags);
  OffsetRect(R1, 2, 0);
  BSDrawSkinText(B.Canvas, Text, R1, Flags);
  OffsetRect(R1, -1, -1);
  BSDrawSkinText(B.Canvas, Text, R1, Flags);
...
begin
  ...
  B.Canvas.Brush.Style := bsClear;
  BSDrawSkinText(B.Canvas, Text, R, Flags);
  BSDrawSkinText(B.Canvas, Text, R, Flags or DT_CALCRECT);
  R1 := ARect;
  BSDrawSkinText(B.Canvas, Text, R1, Flags or DT_CALCRECT);
  if R.Bottom - R.Top > 0 then
    for j := 0 to B.Height - 1 do
    begin
      if (j < R.Top) or (j > R.Bottom) then
...
initialization
  ...
  BS_PlatformIsUnicode := (Win32Platform = VER_PLATFORM_WIN32_NT);
