...
begin
  ...
  BoundsRect := Rect;
  //
  if CheckW2KWXP and FspHint.AlphaBlend
  then
  begin
    SetWindowLong(Handle, GWL_EXSTYLE,
...
begin
  R := Rect(0, 0, 0, 0);
  BSDrawSkinText(Cnvs, S, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
  W := RectWidth(R);
  H := RectHeight(R);
  if FSD <> nil
  then
...
begin
  ...
  then
    ...
    begin
      ...
      R := Rect(0, 0, 0, 0);
      {$IFDEF TNTUNICODE}
      BSDrawSkinText(DrawBuffer.Canvas, Caption, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
      {$ELSE}
      DrawText(Handle, PChar(Caption), -1, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX);
      {$ENDIF}
      W := RectWidth(R);
      ...
      R := Rect(X, Y, X + W, Y + H);
      {$IFDEF TNTUNICODE}
      BSDrawSkinText(DrawBuffer.Canvas, Caption, R, DT_LEFT or DT_NOPREFIX);
      {$ELSE}
      DrawText(Handle, PChar(Caption), -1, R, DT_LEFT or DT_NOPREFIX);
      {$ENDIF}
    end
