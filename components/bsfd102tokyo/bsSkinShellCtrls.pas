...
uses
  ...
  Menus, bsSkinMenus, bsPngImageList, bsEffects;

const
  BS_SID_IShellItemImageFactory = '{bcc18b79-ba16-442f-80c4-8a59c30c463b}';
  BS_IID_IShellItemImageFactory: TGUID = BS_SID_IShellItemImageFactory;
  BS_SID_IExtractImage        = '{BB2E617C-0920-11D1-9A0B-00C04FC2D6C1}';
  BS_IID_IImageList: TGUID= '{46EB5926-582E-4017-9FDF-E8998DAA0950}';
  BS_IID_IExtractImage: TGUID = BS_SID_IExtractImage;
  BS_CLSID_ShellItem: TGUID = (
    D1:$43826d1e; D2:$e718; D3:$42ee; D4:($bc,$55,$a1,$e2,$61,$c3,$7b,$fe));
  BS_IEIT_PRIORITY_NORMAL = $10000000;
  BS_IEIFLAG_OFFLINE = $0008;
  BS_IEIFLAG_QUALITY = $0200;
  BS_IEIFLAG_ORIGSIZE = $0040;
...
type
  ...
  BS_IShellItem = interface(IUnknown)
    ...
    function Compare(const psi: BS_IShellItem; hint: DWORD;
      var piOrder: Integer): HResult; stdcall;
   end;

  BS_IShellItemImageFactory = interface(IUnknown)
    [BS_SID_IShellItemImageFactory]
    function GetImage(size: TSize; flags: UINT; out phbm: HBITMAP): HRESULT; stdcall;
  end;

  BS_IExtractImage = interface(IUnknown)
    [BS_SID_IExtractImage]
...
type
  ...
  TbsThumbnailState = set of (bsthmbIconLoaded, bsthmbThumbnailLoaded);

  TbsThumbnailsThread = class(TThread)
  protected
    FLV: TbsCustomShellListView;
    FStartIndex, FStep: Integer;
    FIconOnly: Boolean;
    procedure Execute; override;
    procedure LoadThumbnails;
    procedure LoadThumbnail(AFolder: TbsShellFolder; AIndex: Integer);
    procedure LoadIcon(AFolder: TbsShellFolder; AIndex: Integer);
  public
    constructor CreateEx(AFLV: TbsCustomShellListView; AStartIndex, AStep: Integer; AIconOnly: Boolean);
  end;
  ...
  public
    ...
    FThumbnail: TbsBitmap;
    FThumbnailState: TbsThumbnailState;
...
{ TShellTreeView }
  ...
  TbsThumbnailOptions = class(TPersistent)
  private
    FHeight: Integer;
    FEnabled: Boolean;
    FWidth: Integer;
...
{ TbsCustomShellListView }
  ...
  private
    FThumbnailOptions: TbsThumbnailOptions;
    ...
    procedure SetPath(const Value: string);
  protected
    procedure UpdateThumbnail(AFolder: TbsShellFolder);
  ...
  public
    ...
    property ThumbnailOptions: TbsThumbnailOptions read
...
{ TShellListView }
 ...
 public
    ...
    NewFolderToolButton: TbsSkinButton;
    DirEdit: TbsSkinEdit;
    //
  ...
  public
    ...
    BottomPanel, ClientPanel: TbsSkinPanel;
    ...
    ToolPanel: TbsSkinToolBar;
    NewFolderToolButton, UpToolButton, BackToolButton: TbsSkinButton;
    StyleToolButton: TbsSkinMenuButton;
  ...
  TbsShellListViewStyle = (bsvsIcon, bsvsSmallIcon, bsvsList, bsvsReport,
  bsvsThumbnails, bsvsBigThumbnails, bsvsVeryBigThumbnails);
  ...
  protected
    ...
    procedure Change;
  public
    ListViewStyle: TbsShellListViewStyle;
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function Execute: Boolean;
  ...
  public
    ...
    BSF: TbsBusinessSkinForm;
    FileListViewPanel,
    BottomPanel, ButtonsPanel, ClientPanel: TbsSkinPanel;
    FLVHScrollBar, FLVVScrollBar: TbsSkinScrollBar;
    FileListView: TbsSkinFileListView;
    FileNameEdit: TbsSkinEdit;
    FilterComboBox: TbsSkinFilterComboBox;
  ...
  public
    ...
    //
    constructor CreateEx(AOwner: TComponent; ACtrlSkinData: TbsSkinData; AToolButtonsTransparent: Boolean;
    AToolBarImageList: TCustomImageList; AShowThumbnails: Boolean);
    destructor Destroy; override;
    procedure FLVChange(Sender: TObject; Item: TListItem; Change: TItemChange);
    procedure FLVPathChange(Sender: TObject);
  ...
  public
    ...
    ShellBox: TbsSkinShellComboBox;
    OpenButton, CancelButton: TbsSkinButton;
    PlayButton, StopButton: TbsSkinButton;
    Drivelabel, OpenFileLabel, FileTypeLabel: TbsSkinStdLabel;
    ToolPanel: TbsSkinToolBar;
    NewFolderToolButton, UpToolButton, BackToolButton: TbsSkinButton;
    StyleToolButton: TbsSkinMenuButton;
...
const
  ...
  var
    BS_SHCreateItemFromIDList: function(pidl: PItemIdList; const riid: TIID; out ppvOut): HResult; stdcall;
    BS_SHGetImageList: function (iImageList: integer; const riid: TGUID; var ppv: Pointer): hResult; stdcall;
    BS_SHGetKnownFolderIDList: function(const rfid: TGUID; dwFlags: DWORD; hToken: THandle; var ppidl: PItemIDList ): HResult; stdcall;

implementation

{$R bsSkinShellCtrls}

uses ShellAPI, ComObj, TypInfo, Consts, Math, bsConst, Masks, {$IFNDEF VER130} MaskUtils,{$ENDIF} bsmessages,
  bsUtils;
...
procedure InvokeContextMenu(Owner: TWinControl; AFolder: TbsShellFolder; X, Y:
Integer);
var
  PIDL: PItemIDList;
  CM: IContextMenu;
...
procedure DoContextMenuVerb(AFolder: TbsShellFolder; Verb: PAnsiChar);
var
  ICI: TCMInvokeCommandInfo;
  CM: IContextMenu;
  PIDL: PItemIDList;
...
constructor TbsThumbnailsThread.CreateEx(AFLV: TbsCustomShellListView; AStartIndex, AStep: Integer; AIconOnly: Boolean);
begin
...
procedure TbsThumbnailsThread.Execute;
begin
  LoadThumbnails;
end;

procedure TbsThumbnailsThread.LoadThumbnails;
var
  I: Integer;
...
procedure TbsThumbnailsThread.LoadThumbnail(AFolder: TbsShellFolder; AIndex: Integer);
var
  FThumbSize: TSize;
  FHBitmap: HBitmap;
  FShellItem: BS_IShellItem;
  FItemImageFactory: BS_IShellItemImageFactory;
  FShellFolder: IShellFolder;
  FTempPidl: PItemIDList;
  FExtractor: BS_IExtractImage;
  FPriority: Cardinal;
  FFlags: Cardinal;
...
begin
  ...
  if not (bsthmbIconLoaded in AFolder.FThumbnailState) then
  begin
    AFolder.FThumbnailState := AFolder.FThumbnailState + [bsthmbIconLoaded];
...
begin
  ...
  if FLV.FIsVistaOrLater and
    (@BS_SHCreateItemFromIDList <> nil) and
    (BS_SHCreateItemFromIDList(AFolder.FFullPIDL, BS_CLSID_ShellItem, FShellItem) = S_Ok) and
    (FShellItem.QueryInterface(BS_IID_IShellItemImageFactory, FItemImageFactory) = S_OK) and
    ...
    try
      if (FShellFolder <> nil) and
         (FShellFolder.GetUIObjectOf(0, 1, FTempPidl,
             BS_IID_IExtractImage, nil, FExtractor) = S_OK) then
      begin
        FPriority := BS_IEIT_PRIORITY_NORMAL;
        FFlags := BS_IEIFLAG_OFFLINE or BS_IEIFLAG_QUALITY or BS_IEIFLAG_ORIGSIZE;
...
procedure TbsThumbnailsThread.LoadIcon(AFolder: TbsShellFolder; AIndex: Integer);
var
  FThumbSize: TSize;
  FHBitmap: HBitmap;
  FShellItem: BS_IShellItem;
...
begin
  if FLV.FStopFetch or (bsthmbThumbnailLoaded in AFolder.FThumbnailState) or
...
destructor TbsCustomShellTreeView.Destroy;
begin
...
{      if Assigned(OnEdited) then OnEdited(Self, Node, S);
      if ( Node <> nil ) and TbsShellFolder(Node.Data).Rename(S) then
        Node.Text := S;}
 {     if (Selected <> nil) and (not DirectoryExists(S)) and
         (Pos(':\', S) <> 0)
      then
        begin
          RenameFile(F2, F1);
...
constructor TbsThumbnailOptions.Create(AOwner: TPersistent);
begin
  inherited Create;
...
procedure TbsThumbnailOptions.SetValues(AEnabled: Boolean; AWidth, AHeight: Integer);
begin
  FEnabled := AEnabled;
  FWidth := AWidth;
  FHeight := AHeight;
...
procedure TbsThumbnailOptions.Assign(Source: TPersistent);
begin
  if Source is TbsThumbnailOptions then
    with TbsThumbnailOptions(Source) do
    begin
      Self.FHeight := Height;
      Self.FWidth := Width;
...
procedure TbsThumbnailOptions.Changed;
begin
  DoChange;
end;

procedure TbsThumbnailOptions.DoChange;
begin
  if Assigned(FOnChange) and FEnabled then
    FOnChange(Self);
end;

procedure TbsThumbnailOptions.DoEnabledChange;
begin
  if Assigned(FOnEnabledChange) then
    FOnEnabledChange(Self);
end;

procedure TbsThumbnailOptions.SetHeight(const Value: Integer);
begin
...
procedure TbsThumbnailOptions.SetEnabled(const Value: Boolean);
begin
...
procedure TbsThumbnailOptions.SetWidth(const Value: Integer);
begin
...
begin
  ...
  FThumbnailOptions := TbsThumbnailOptions.Create(Self);
  ...
  FExtraLargeImages := FLargeImages;
  if @BS_SHGetImageList <> nil then
    BS_SHGetImageList(SHIL_EXTRALARGE, BS_IID_IImageList, Pointer(FExtraLargeImages));
...
procedure TbsCustomShellListView.ClearThreads;
var
  I: Integer;
begin
  FStopFetch := True;
  ...
  begin
    ...
    begin
      TbsThumbnailsThread(FThumbsThreadList[I]).Terminate;
      TbsThumbnailsThread(FThumbsThreadList[I]).WaitFor;
      TbsThumbnailsThread(FThumbsThreadList[I]).Free;
    end;
...
function TbsCustomShellListView.GetThreadCount: Integer;
begin
  Result := 3;
...
function TbsCustomShellListView.IsThumbnailView: Boolean;
begin
  Result := CheckWXP and (ViewStyle = vsIcon) and FThumbnailOptions.Enabled;
end;

procedure TbsCustomShellListView.UpdateThumbnails;
begin
...
procedure TbsCustomShellListView.OnThumbnailOptionsEnabledChange(Sender: TObject);
begin
...
procedure TbsCustomShellListView.OnThumbnailOptionsChange(Sender: TObject);
begin
  UpdateThumbnails;
  RootChanged;
end;

 procedure TbsCustomShellListView.NewAdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
...
procedure TbsCustomShellListView.UpdateThumbnail(AFolder: TbsShellFolder);
var
  FTempBitmap: TBitmap;
begin
  if AFolder <> nil then
  begin
    if (AFolder.FThumbnailHBitmap > 0) and not (bsthmbThumbnailLoaded in AFolder.FThumbnailState)
    then
    begin
      if AFolder.FThumbnail.Width * AFolder.FThumbnail.Height <> 0 then
      begin
        AFolder.FThumbnail.Free;
        AFolder.FThumbnail := TbsBitmap.Create;
      end;
      ...
      begin
        if not AFolder.IsFolder then
          ...
          AFolder.FThumbnail.CheckRGBA;
      end
      else
        AFolder.FThumbnail.CheckingAlphaBlend;
      FTempBitmap.Free;
      AFolder.FThumbnailState := AFolder.FThumbnailState + [bsthmbThumbnailLoaded];
      AFolder.FThumbnailHBitmap := 0;
    end
    else
    if (AFolder.FIconHBitmap > 0) and not (bsthmbIconLoaded in AFolder.FThumbnailState) and
       not (bsthmbThumbnailLoaded in AFolder.FThumbnailState)  then
    begin
      AFolder.FThumbnail.AlphaBlend := True;
      FTempBitmap := TBitmap.Create;
...
procedure TbsCustomShellListView.DrawThumbnailImage(AItem: TListItem;
  ACanvas: TCanvas; ARect: TRect);
var
  AFolder: TbsShellFolder;
  CX, CY, W, H: Integer;
...
begin
  ...
  begin
    ...
    if  (FThumbnailIndex <> -1) and
        (bsthmbIconLoaded in FThumbnailState) and not (bsthmbThumbnailLoaded in FThumbnailState)
    then
    begin
      ...
      ImageList_GetIconSize(FExtraLargeImages, W, H);
      CX := ARect.Left + RectWidth(ARect) div 2 - W div 2;
      CY := ARect.Top + RectHeight(ARect) div 2 - H div 2;
      ImageList_DrawEx(FExtraLargeImages, FThumbnailIndex, ACanvas.Handle,
        CX, CY, W, H, CLR_NONE, CLR_NONE, ILD_NORMAL);
    end
    else
...
function TbsCustomShellListView.GetSelectedPath: String;
const
  CSIDL_MYVIDEO = $0e;
  CSIDL_MYPICTURES = $27;
  CSIDL_MYMUSIC = $0d;
...
begin
  ...
  then
    begin
      ...
      then
        begin
          TbsShellFolder(Item1).LoadColumnDetails(CompareFolder, ListViewHandle, 4);
          TbsShellFolder(Item2).LoadColumnDetails(CompareFolder, ListViewHandle, 4);
        end;  
      if (SortColumn = 0) or
         ((SortColumn <> 0) and (Pos({$IFDEF VER240_UP}FormatSettings.{$ENDIF}TimeSeparator, TbsShellFolder(Item1).Details[4]) <> 0))
      then
      begin
      if TbsShellFolder(Item1).FDetailsEx then
      begin
       case SortColumn of
      0:
        begin
          if TbsShellFolder(Item1).IsFolder and not TbsShellFolder(Item2).IsFolder
          then
            Result := -1
          else
          if TbsShellFolder(Item2).IsFolder and not TbsShellFolder(Item1).IsFolder
          then
            Result := 1
          else
            Result := AnsiCompareText(TbsShellFolder(Item1).DisplayName,
                                      TbsShellFolder(Item2).DisplayName);
        end;
      2:
        begin
          if TbsShellFolder(Item1).IsFolder and not TbsShellFolder(Item2).IsFolder
          ...
          then
            ...
            begin
              if TbsShellFolder(Item1).FDetails.Count > 0
              then
                S1 := TbsShellFolder(Item1).Details[2]
              else
                S1 := '';
              if TbsShellFolder(Item2).FDetails.Count > 0
              then
                S2 := TbsShellFolder(Item2).Details[2]
              else
                S2 := '';
      ...
      3:
        begin
          ...
          if not TbsShellFolder(Item1).IsFolder and not TbsShellFolder(Item2).IsFolder
          then
            begin
              GetDetailsOf(TbsShellFolder(Item1), Details1);
              GetDetailsOf(TbsShellFolder(Item2), Details2);
...
procedure  TbsCustomShellListView.ColClick(Column: TListColumn);
begin
  inherited;
...
var
  ...
  W, I: Integer;
  R: TRect;
  FThumbsThread: TbsThumbnailsThread;
...
begin
  ...
  then
   ...
   begin
     ...
     begin
       FThumbsThread := TbsThumbnailsThread.CreateEx(Self, I, FThreadCount, False);
       FThumbsThreadList.Add(FThumbsThread);
       FThumbsThread.Resume;
     end;
     for I := 0 to FThreadCount - 1 do
     begin
       FThumbsThread := TbsThumbnailsThread.CreateEx(Self, I, FThreadCount, True);
       FThumbsThreadList.Add(FThumbsThread);
       FThumbsThread.Resume;
     end;
   end;
...
procedure TbsCustomShellListView.SetRootDirectly(const Value: TRoot);
begin
  FOldRoot := FRoot;
  FRoot := Value;
  CreateRoot;
...
begin
  ...
  PngImgList := TbsPngImageList.Create(AOwner);
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_NEWFOLDER_PNG');
  ...
  then
    begin
      ...
      NewFolderToolButton := TbsSkinButton.Create(Self);
...
begin
  ...
  FDlgFrm := TbsSelDirDlgForm.CreateEx(Application, CtrlSkinData, FShowToolBar,
    FToolButtonsTransparent, FToolBarImageList, FToolButtonImageIndex,
    FTreeShowLines, FTreeButtonImageList,
  ...
  try
    ...
    BSF.MinClientWidth := 300;
    BSF.MinClientHeight := 300;
    BSF.AlphaBlend := Self.AlphaBlend;
    BSF.AlphaBlendAnimation := AlphaBlendAnimation;
    BSF.AlphaBlendValue := Self.AlphaBlendValue;
    //
...
begin
  ...
  PngImgList := TbsPngImageList.Create(AOwner);
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_BACK_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_UP_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_NEWFOLDER_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_LVSTYLE_PNG');
  ...
  BackToolButton := TbsSkinButton.Create(Self);
  ...
  UpToolButton := TbsSkinButton.Create(Self);
  ...
  begin
    ...
    begin
      ...
      then
        ...
        Caption := BS_MSG_LV_THUMBNAILS;
      if Caption = '' then Caption := BS_MSG_LV_THUMBNAILS;
      GroupIndex := 1;
      RadioItem := True;            
    ...
    begin
      ...
      then
        ...
        Caption := BS_MSG_LV_BIGTHUMBNAILS;
      if Caption = '' then Caption := BS_MSG_LV_BIGTHUMBNAILS;
      GroupIndex := 1;
      RadioItem := True;            
    ...
    begin
      ...
      then
        ...
        Caption := BS_MSG_LV_VERYBIGTHUMBNAILS;
      if Caption = '' then Caption := BS_MSG_LV_VERYBIGTHUMBNAILS;
      GroupIndex := 1;
      RadioItem := True;
  ...
  StyleToolButton := TbsSkinMenuButton.Create(Self);
  ...
  ButtonsPanel := TbsSkinPanel.Create(Self);
  ...
  ClientPanel := TbsSkinPanel.Create(Self);
  with ClientPanel do
  begin
    Parent := Self;
    Align := alClient;
...
begin
  if @BS_SHCreateItemFromIDList <> nil then
    BS_SHGetKnownFolderIDList(FOLDERID_LIBRARIES, 0, 0, Result)
  else
    Result := nil;
end;

procedure TbsOpenDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TbsShellFolder;
  NewPIDL: PItemIDList;
begin
  if (TbsSkinButton(Sender).Tag = Ord(rfPersonal)) and IsVistaOS then
  begin
    NewPIDL := GetIdList_Libraries;
    if NewPIDL <> nil then
      FileListView.SetPathFromID(NewPIDL)
    else
      FileListView.SetRootDirectly(GetEnumName(TypeInfo(TRootFolder), TbsSkinButton(Sender).Tag));
  end
  else
    FileListView.SetRootDirectly(GetEnumName(TypeInfo(TRootFolder), TbsSkinButton(Sender).Tag));
...
procedure TbsOpenDlgForm.CreateShellButtons;

  procedure CreateBtn(Folder: TRootFolder);
  var
    FButton: TbsSkinButton;
    NewPIDL: PItemIDList;
  begin
    FButton := TbsSkinButton.Create(Self);
...
procedure TbsOpenDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
...
procedure TbsOpenDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
...
procedure TbsOpenDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
...
procedure TbsOpenDlgForm.InitHistory;
var
  ID: PItemIDList;
begin
...
begin
  ...
  try
    ...
    BSF.MinWidth := FDialogMinWidth;
    BSF.MinHeight := FDialogMinHeight;
    BSF.MinClientHeight := 350;
    BSF.MinClientWidth := 550;
    // alpha blend
    BSF.AlphaBlend := Self.AlphaBlend;
    BSF.AlphaBlendAnimation := AlphaBlendAnimation;
    BSF.AlphaBlendValue := Self.AlphaBlendValue;
    ...
    ((ListViewStyle = bsvsThumbnails) or
    (ListViewStyle = bsvsBigThumbnails) or
    (ListViewStyle = bsvsVeryBigThumbnails))
    then
      ListViewStyle := bsvsList;
    //
   ...
   if FDialogWidth > 0
    ...
    begin
      case FileListView.ThumbnailOptions.Width of
        96: ListViewStyle := bsvsThumbnails;
        128: ListViewStyle := bsvsBigThumbnails;
        256: ListViewStyle := bsvsVeryBigThumbnails;
      end;
    end
    else
      case FileListView.ViewStyle of
        vsIcon:
          ListViewStyle := bsvsIcon;
        vsSmallIcon:
          ListViewStyle := bsvsSmallIcon;
        vsReport:
          ListViewStyle := bsvsReport;
        vsList:
          ListViewStyle := bsvsList;
      end;
...
begin
  ...
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_LVSTYLE_PNG');
  with TbsPngImageItem(PngImgList.PngImages.Add) do
    PngImage.LoadFromResourceName(HInstance, 'BS_STRETCH_PNG');
 ...
 IconMenuItem := TMenuItem.Create(Self);
  ...
  StretchButton := TbsSkinButton.Create(Self);
...
procedure TbsOpenPictureDlgForm.CreateShellButtons;
...
procedure TbsOpenPictureDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TbsShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TbsOpenPictureDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenPictureDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenPictureDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenPictureDlgForm.InitHistory;
var
  ID: PItemIDList;
begin
  ID := CopyPIDL(FileListView.RootFolder.AbsoluteID);
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TbsOpenPictureDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent,
...
begin
  ...
  with PreviewBSF do
  begin
    PreviewBSF.BorderIcons := PreviewBSF.BorderIcons - [biSystemMenu];
    SkinData := PreviewSkinData;
    PreviewMode := True;
  end;
...
procedure TbsOpenSkinDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TbsShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TbsOpenSkinDlgForm.CreateShellButtons;
...
procedure TbsOpenSkinDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenSkinDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenSkinDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenSkinDlgForm.InitHistory;
var
  ID: PItemIDList;
begin
  ID := CopyPIDL(FileListView.RootFolder.AbsoluteID);
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TbsOpenSkinDlgForm.CreateEx(Application, CtrlSkinData, FToolButtonsTransparent,
   FToolBarImageList, FShowThumbnails);
  with FDlgFrm do
  try
    Caption := Self.Title;
...
procedure TbsOpenPreviewDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TbsShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TbsOpenPreviewDlgForm.CreateShellButtons;
...
procedure TbsOpenPreviewDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenPreviewDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenPreviewDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenPreviewDlgForm.InitHistory;
var
  ID: PItemIDList;
begin
  ID := CopyPIDL(FileListView.RootFolder.AbsoluteID);
...
Drivelabel := TbsSkinStdLabel.Create(Self);
  ...
  PlayButton := TbsSkinButton.Create(Self);
  with PlayButton do
  begin
    Align := alLeft;
    Parent := SoundPanel;
  ...
  StopButton := TbsSkinButton.Create(Self);
  with StopButton do
  begin
    Align := alLeft;
    Parent := SoundPanel;
...
procedure TbsOpenSoundDlgForm.ShellButtonClick(Sender: TObject);
var
  F: TbsShellFolder;
  NewPIDL: PItemIDList;
begin
...
procedure TbsOpenSoundDlgForm.CreateShellButtons;
...
procedure TbsOpenSoundDlgForm.BigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 128, 128);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenSoundDlgForm.VeryBigThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 256, 256);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenSoundDlgForm.ThumbnailsItemClick(Sender: TObject);
begin
  FileListView.ThumbnailOptions.SetValues(True, 96, 96);
  FileListView.ViewStyle := vsIcon;
  FileListView.UpdateThumbnails;
...
procedure TbsOpenSoundDlgForm.InitHistory;
var
  ID: PItemIDList;
begin
  ID := CopyPIDL(FileListView.RootFolder.AbsoluteID);
...
initialization
  ...
  if Shell32DLL <> 0 then begin
    BS_SHCreateItemFromIDList := GetProcAddress(Shell32DLL, 'SHCreateItemFromIDList');
    BS_SHGetImageList := GetProcAddress(Shell32DLL, 'SHGetImageList');
    BS_SHGetKnownFolderIDList := GetProcAddress(Shell32DLL, 'SHGetKnownFolderIDList');
  end;
