...
{$IFDEF VER200}
 ...
 type
   TsePngImage = class(TPngImage);

{$ELSE}

uses
 Windows, Classes, Graphics, SysUtils, se_zlib;
...
type
  ...
  TsePngTransparencyMode = (bsptmNone, bsptmBit, bsptmPngLayerial);
  pCardinal = ^Cardinal;
  pRGBPixel = ^TRGBPixel;
  ...
  protected
    ...
    property Owner: TsePngImage read fOwner;
  public
    constructor Create(AOwner: TsePngImage);
    destructor Destroy; override;
    property Count: Cardinal read fCount write SetSize;
  end;

  TsePngLayer = class;
  TsePngLayerClass = class of TsePngLayer;

  TsePngList = class(TsePngPointerList)
  private
    function GetItem(Index: Cardinal): TsePngLayer;
  public
    function FindPngLayer(PngLayerClass: TsePngLayerClass): TsePngLayer;
    procedure RemovePngLayer(PngLayer: TsePngLayer); overload;
    function Add(PngLayerClass: TsePngLayerClass): TsePngLayer;
    function ItemFromClass(PngLayerClass: TsePngLayerClass): TsePngLayer;
    property Item[Index: Cardinal]: TsePngLayer read GetItem;
  end;

  TsePngLayerIHDR = class;
  TsePngLayerpHYs = class;
  TseInterlaceMethod = (bsimNone, bsimAdam7);
  TseCompressionLevel = 0..9;
  TsePngFilter = (bspfNone, bspfSub, bspfUp, bspfAverage, bspfPaeth);
  TsePngFilters = set of TsePngFilter;

  TsePngImage = class(TGraphic)
  private
    fMaxIdatSize: Integer;
    fInterlaceMethod: TseInterlaceMethod;
    fPngLayerList: TsePngList;
    fCanvas: TCanvas;
    fFilters: TsePngFilters;
    fCompressionLevel: TseCompressionLevel;
    ...
    function GetTransparencyMode: TsePngTransparencyMode;
  ...
  protected
    ...
    procedure AssignPNG(Source: TsePngImage);
    function GetEmpty: Boolean; override; 
    function GetHeader: TsePngLayerIHDR;
  ...
  public
    ...
    property Header: TsePngLayerIHDR read GetHeader;
    property TransparencyMode: TsePngTransparencyMode read GetTransparencyMode;

    property Width: Integer read GetWidth;
    property Height: Integer read GetHeight;
    property InterlaceMethod: TseInterlaceMethod read fInterlaceMethod
      write fInterlaceMethod;
    property Filters: TsePngFilters read fFilters write fFilters;
    property MaxIdatSize: Integer read fMaxIdatSize write SetMaxIdatSize;
    property Empty: Boolean read GetEmpty;
    property CompressionLevel: TseCompressionLevel read fCompressionLevel
      write fCompressionLevel;
    property PngLayers: TsePngList read fPngLayerList;
    property Pixels[const X, Y: Integer]: TColor read GetPixels write SetPixels;
  end;

  TsePngLayerName = Array[0..3] of Char;
  ...
  public
    ...
    procedure Assign(Source: TsePngLayer); virtual;
    constructor Create(Owner: TsePngImage); virtual;
    destructor Destroy; override;
    function LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
  ...
  TsePngLayerIEND = class(TsePngLayer); 
  ...
  TsePngLayerIHDR = class(TsePngLayer)
  private
    ImageHandle: HBitmap;
    ImageDC: HDC;
  ...
  public
    ...
    constructor Create(Owner: TsePngImage); override;
    destructor Destroy; override;
    procedure Assign(Source: TsePngLayer); override;
  ...
  TsePngLayerpHYs = class(TsePngLayer)
  private
  ...
  TsePngLayergAMA = class(TsePngLayer)
  private
  ...
  TsePngLayerPLTE = class(TsePngLayer)
  protected
    fCount: Integer;
  private
  ...
  TsePngLayertRNS = class(TsePngLayer)
  private
  ...
  TsePngLayerIDAT = class(TsePngLayer)
  private
    Header: TsePngLayerIHDR;
  ...
  TsePngLayertIME = class(TsePngLayer)
  private
    fYear: Word;
  ...
  TsePngLayertEXt = class(TsePngLayer)
  private
  ...
  TsePngLayerzTXt = class(TsePngLayertEXt)
    function LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
      Size: Integer): Boolean; override;
    function SaveToStream(Stream: TStream): Boolean; override;
  end;

procedure RegisterPngLayer(PngLayerClass: TsePngLayerClass);
...
var
  PngLayerClasses: TsePngPointerList;
...
type
  pPngLayerClassInfo = ^TsePngLayerClassInfo;
  TsePngLayerClassInfo = record
    ClassName: TsePngLayerClass;
  end;
...
begin
  if PngLayerClasses = nil then PngLayerClasses := TsePngPointerList.Create(nil);
  new(NewClass);
...
begin
  RegisterPngLayer(TsePngLayerIEND);
  RegisterPngLayer(TsePngLayerIHDR);
  RegisterPngLayer(TsePngLayerIDAT);
  RegisterPngLayer(TsePngLayerPLTE);
  RegisterPngLayer(TsePngLayergAMA);
  RegisterPngLayer(TsePngLayertRNS);
  RegisterPngLayer(TsePngLayerpHYs);
  RegisterPngLayer(TsePngLayertIME);
  RegisterPngLayer(TsePngLayertEXt);
  RegisterPngLayer(TsePngLayerzTXt);
end;

function CreateClassPngLayer(Owner: TsePngImage; Name: TsePngLayerName): TsePngLayer;
var
  i       : Integer;
  NewPngLayer: TsePngLayerClass;
begin
  NewPngLayer := TsePngLayer;
...
function ZLIBInitDeflate(Stream: TStream;
  Level: TseCompressionlevel; Size: Cardinal): TZStreamRec2;
begin
  Fillchar(Result, SIZEOF(TZStreamRec2), #0);
  with Result do
  begin
...
constructor TsePngPointerList.Create(AOwner: TsePngImage);
begin
  inherited Create;
  fOwner := AOwner;
  fMemory := nil;
...
function TsePngPointerList.Remove(Value: Pointer): Pointer;
var
  I, Position: Integer;
begin
  Position := -1;
...
procedure TsePngPointerList.Add(Value: Pointer);
begin
  Count := Count + 1;
  Item[Count - 1] := Value;
end;

destructor TsePngPointerList.Destroy;
begin
  if fMemory <> nil then
...
function TsePngPointerList.GetItem(Index: Cardinal): Pointer;
begin
...
procedure TsePngPointerList.Insert(Value: Pointer; Position: Cardinal);
begin
...
procedure TsePngPointerList.SetItem(Index: Cardinal; const Value: Pointer);
begin
  if (Index <= Count - 1) then
    fMemory[Index] := Value
end;

procedure TsePngPointerList.SetSize(const Size: Cardinal);
begin
...
function TsePngList.FindPngLayer(PngLayerClass: TsePngLayerClass): TsePngLayer;
var
  i: Integer;
begin
  Result := nil;
...
procedure TsePngList.RemovePngLayer(PngLayer: TsePngLayer);
begin
  Remove(PngLayer);
  PngLayer.Free
end;

function TsePngList.Add(PngLayerClass: TsePngLayerClass): TsePngLayer;
var
  IHDR: TsePngLayerIHDR;
  IEND: TsePngLayerIEND;

  IDAT: TsePngLayerIDAT;
  PLTE: TsePngLayerPLTE;
begin
  Result := nil; 
  if ((PngLayerClass = TsePngLayerIHDR) or (PngLayerClass = TsePngLayerIDAT) or
    (PngLayerClass = TsePngLayerPLTE) or (PngLayerClass = TsePngLayerIEND)) and not
    (Owner.BeingCreated)
    then
      begin
      end
  else if ((PngLayerClass = TsePngLayergAMA) and (ItemFromClass(TsePngLayergAMA) <> nil)) or
     ((PngLayerClass = TsePngLayertRNS) and (ItemFromClass(TsePngLayertRNS) <> nil)) or
     ((PngLayerClass = TsePngLayerpHYs) and (ItemFromClass(TsePngLayerpHYs) <> nil)) then
     begin
     end
  else if ((ItemFromClass(TsePngLayerIEND) = nil) or
    (ItemFromClass(TsePngLayerIHDR) = nil)) and not Owner.BeingCreated then
    begin
    end
  else
  begin
    IHDR := ItemFromClass(TsePngLayerIHDR) as TsePngLayerIHDR;
    IEND := ItemFromClass(TsePngLayerIEND) as TsePngLayerIEND;
    Result := PngLayerClass.Create(Owner);
    if (PngLayerClass = TsePngLayergAMA) or (PngLayerClass = TsePngLayerpHYs) or
      (PngLayerClass = TsePngLayerPLTE) then
      Insert(Result, IHDR.Index + 1)
    else if (PngLayerClass = TsePngLayerIEND) then
      Insert(Result, Count)
    else if (PngLayerClass = TsePngLayerIHDR) then
      Insert(Result, 0)
    else if (PngLayerClass = TsePngLayertRNS) then
    begin
      IDAT := ItemFromClass(TsePngLayerIDAT) as TsePngLayerIDAT;
      PLTE := ItemFromClass(TsePngLayerPLTE) as TsePngLayerPLTE;
...
function TsePngList.GetItem(Index: Cardinal): TsePngLayer;
begin
  Result := inherited GetItem(Index);
end;

function TsePngList.ItemFromClass(PngLayerClass: TsePngLayerClass): TsePngLayer;
var
  i: Integer;
begin
  Result := nil;
...
procedure TsePngLayer.ResizeData(const NewSize: Cardinal);
begin
  fDataSize := NewSize;
  ReallocMem(fData, NewSize + 1);
end;

function TsePngLayer.GetIndex: Integer;
var
  i: Integer;
begin
  Result := -1; 
...
function TsePngLayer.GetHeader: TsePngLayerIHDR;
begin
  Result := Owner.PngLayers.Item[0] as TsePngLayerIHDR;
end;

procedure TsePngLayer.Assign(Source: TsePngLayer);
begin
  fName := Source.fName;
...
constructor TsePngLayer.Create(Owner: TsePngImage);
var
  PngLayerName: String;
begin
  inherited Create;
  PngLayerName := System.Copy(ClassName, Length('TsePngLayer') + 1, Length(ClassName));
...
destructor TsePngLayer.Destroy;
begin
  FreeMem(fData, fDataSize + 1);
  inherited Destroy;
end;

function TsePngLayer.GetPngLayerName: String;
begin
  Result := fName
end;

class function TsePngLayer.GetName: String;
begin
  Result := System.Copy(ClassName, Length('TsePngLayer') + 1, Length(ClassName));
end;

function TsePngLayer.SaveData(Stream: TStream): Boolean;
var
...
function TsePngLayer.SaveToStream(Stream: TStream): Boolean;
begin
  Result := SaveData(Stream)
end;

function TsePngLayer.LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
...
function TsePngLayertIME.LoadFromStream(Stream: TStream;
  const PngLayerName: TsePngLayerName; Size: Integer): Boolean;
begin
...
procedure TsePngLayertIME.Assign(Source: TsePngLayer);
begin
  fYear := TsePngLayertIME(Source).fYear;
  fMonth := TsePngLayertIME(Source).fMonth;
  fDay := TsePngLayertIME(Source).fDay;
  fHour := TsePngLayertIME(Source).fHour;
  fMinute := TsePngLayertIME(Source).fMinute;
  fSecond := TsePngLayertIME(Source).fSecond;
end;

function TsePngLayertIME.SaveToStream(Stream: TStream): Boolean;
begin
  ResizeData(7);  
...
function TsePngLayerzTXt.LoadFromStream(Stream: TStream;
  const PngLayerName: TsePngLayerName; Size: Integer): Boolean;
var
  ErrorOutput: String;
...
function TsePngLayerztXt.SaveToStream(Stream: TStream): Boolean;
var
  Output: Pointer;
  OutputSize: Integer;
  ErrorOutput: String;
...
procedure TsePngLayertEXt.Assign(Source: TsePngLayer);
begin
  fKeyword := TsePngLayertEXt(Source).fKeyword;
  fText := TsePngLayertEXt(Source).fText;
end;

function TsePngLayertEXt.LoadFromStream(Stream: TStream;
  const PngLayerName: TsePngLayerName; Size: Integer): Boolean;
begin
  Result := inherited LoadFromStream(Stream, PngLayerName, Size);
...
function TsePngLayertEXt.SaveToStream(Stream: TStream): Boolean;
begin
...
constructor TsePngLayerIHDR.Create(Owner: TsePngImage);
begin
  ImageHandle := 0;
  ImagePalette := 0;
  ImageDC := 0;
...
destructor TsePngLayerIHDR.Destroy;
begin
  FreeImageData();
  inherited Destroy;
end;
...
procedure TsePngLayerIHDR.Assign(Source: TsePngLayer);
begin
  if Source is TsePngLayerIHDR then
  begin
    IHDRData := TsePngLayerIHDR(Source).IHDRData;
    PrepareImageData();
    CopyMemory(ImageData, TsePngLayerIHDR(Source).ImageData,
      BytesPerRow * Integer(Height));
    CopyMemory(ImageAlpha, TsePngLayerIHDR(Source).ImageAlpha,
      Integer(Width) * Integer(Height));
    BitmapInfo.bmiColors := TsePngLayerIHDR(Source).BitmapInfo.bmiColors;
    CopyPalette(TsePngLayerIHDR(Source).ImagePalette, ImagePalette);
  end;
end;

procedure TsePngLayerIHDR.FreeImageData;
begin
  {Free old image data}
...
function TsePngLayerIHDR.LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
  Size: Integer): Boolean;
begin
  Result := inherited LoadFromStream(Stream, PngLayerName, Size);
  ...
  Owner.InterlaceMethod := TseInterlaceMethod(IHDRData.InterlaceMethod);
  PrepareImageData();
end;

function TsePngLayerIHDR.SaveToStream(Stream: TStream): Boolean;
begin
...
function TsePngLayerIHDR.CreateGrayscalePalette(Bitdepth: Integer): HPalette;
var
  j: Integer;
...
procedure TsePngLayerIHDR.PaletteToDIB(Palette: HPalette);
var
  j: Integer;
  palEntries: TMaxLogPalette;
begin
...
procedure TsePngLayerIHDR.PrepareImageData();
...
procedure TsePngLayertRNS.SetTransparentColor(const Value: ColorRef);
var
  i: Byte;
  LookColor: TRGBQuad;
begin
...
function TsePngLayertRNS.GetTransparentColor: ColorRef;
var
  PalettePngLayer: TsePngLayerPLTE;
  i: Integer;
  Value: Byte;
begin
  Result := 0;
  with Header do
    case ColorType of
      ...
      COLOR_PALETTE:
      begin
        PalettePngLayer := Owner.PngLayers.ItemFromClass(TsePngLayerPLTE) as TsePngLayerPLTE;
...
function TsePngLayertRNS.SaveToStream(Stream: TStream): Boolean;
begin
...
procedure TsePngLayertRNS.Assign(Source: TsePngLayer);
begin
  CopyMemory(@PaletteValues[0], @TsePngLayerTrns(Source).PaletteValues[0], 256);
  fBitTransparency := TsePngLayerTrns(Source).fBitTransparency;
  inherited Assign(Source);
end;

function TsePngLayertRNS.LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
  Size: Integer): Boolean;
var
  i, Differ255: Integer;
begin
...
procedure TsePngLayerIDAT.PreparePalette;
var
  Entries: Word;
  j      : Integer;
  palEntries: TMaxLogPalette;
...
function TsePngLayerIDAT.IDATZlibRead(var ZLIBStream: TZStreamRec2;
...
procedure TsePngLayerIDAT.CopyInterlacedRGB8(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.CopyInterlacedRGB16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.CopyInterlacedPalette148(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
const
...
procedure TsePngLayerIDAT.CopyInterlacedPalette2(const Pass: Byte; Src, Dest,
  Trans, Extra: pChar);
var
  CurBit, Col: Integer;
  Dest2: PChar;
...
procedure TsePngLayerIDAT.CopyInterlacedGray2(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  CurBit, Col: Integer;
  Dest2: PChar;
...
procedure TsePngLayerIDAT.CopyInterlacedGrayscale16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.CopyInterlacedRGBAlpha8(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.CopyInterlacedRGBAlpha16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.CopyInterlacedGrayscaleAlpha8(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.CopyInterlacedGrayscaleAlpha16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.DecodeInterlacedAdam7(Stream: TStream;
  var ZLIBStream: TZStreamRec2; const Size: Integer; var crcfile: Cardinal);
var
  CurrentPass: Byte;
...
procedure TsePngLayerIDAT.CopyNonInterlacedRGB8(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedRGB16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedPalette148(
  Src, Dest, Trans, Extra: pChar);
begin
  CopyMemory(Dest, Src, Row_Bytes);
end;

procedure TsePngLayerIDAT.CopyNonInterlacedGray2(
  Src, Dest, Trans, Extra: pChar);
var
  i: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedPalette2(
  Src, Dest, Trans, Extra: pChar);
var
  i: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedGrayscale16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedRGBAlpha8(
  Src, Dest, Trans, Extra: pChar);
var
  i: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedRGBAlpha16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedGrayscaleAlpha8(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsePngLayerIDAT.CopyNonInterlacedGrayscaleAlpha16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsePngLayerIDAT.DecodeNonInterlaced(Stream: TStream;
  var ZLIBStream: TZStreamRec2; const Size: Integer; var crcfile: Cardinal);
var
  j: Cardinal;
  Trans, Data, Extra: pChar;
...
procedure TsePngLayerIDAT.FilterRow;
var
  pp: Byte;
...
function TsePngLayerIDAT.LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
  Size: Integer): Boolean;
var
  ...
  CRCFile  : Cardinal;
begin
  Header := Owner.PngLayers.Item[0] as TsePngLayerIHDR;
...
function TsePngLayerIDAT.SaveToStream(Stream: TStream): Boolean;
var
  ZLIBStream : TZStreamRec2;
begin
  Header := Owner.PngLayers.Item[0] as TsePngLayerIHDR;
...
procedure TsePngLayerIDAT.IDATZlibWrite(var ZLIBStream: TZStreamRec2;
...
procedure TsePngLayerIDAT.FinishIDATZlib(var ZLIBStream: TZStreamRec2);
begin
  with ZLIBStream, ZLIBStream.ZLIB do
  begin
    next_in := nil;
...
procedure TsePngLayerIDAT.EncodeNonInterlacedRGB8(Src, Dest, Trans: pChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TsePngLayerIDAT.EncodeNonInterlacedRGB16(Src, Dest, Trans: pChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TsePngLayerIDAT.EncodeNonInterlacedPalette148(Src, Dest, Trans: pChar);
begin
  CopyMemory(Dest, Src, Row_Bytes);
end;

procedure TsePngLayerIDAT.EncodeNonInterlacedGrayscale16(Src, Dest, Trans: pChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TsePngLayerIDAT.EncodeNonInterlacedRGBAlpha8(Src, Dest, Trans: pChar);
var
  i: Integer;
begin
  FOR i := 1 TO ImageWidth do
...
procedure TsePngLayerIDAT.EncodeNonInterlacedRGBAlpha16(Src, Dest, Trans: pChar);
var
  i: Integer;
begin
  FOR i := 1 TO ImageWidth do
...
procedure TsePngLayerIDAT.EncodeNonInterlacedGrayscaleAlpha8(
  Src, Dest, Trans: pChar);
var
  i: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeNonInterlacedGrayscaleAlpha16(
  Src, Dest, Trans: pChar);
var
  i: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeNonInterlaced(Stream: TStream;
  var ZLIBStream: TZStreamRec2);
var
  j: Cardinal;
  Data, Trans: PChar;
...
procedure TsePngLayerIDAT.EncodeInterlacedRGB8(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedRGB16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedPalette148(const Pass: Byte;
  Src, Dest, Trans: pChar);
const
  BitTable: Array[1..8] of Integer = ($1, $3, 0, $F, 0, 0, 0, $FF);
  StartBit: Array[1..8] of Integer = (7 , 0 , 0, 4,  0, 0, 0, 0);
...
procedure TsePngLayerIDAT.EncodeInterlacedGrayscale16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedRGBAlpha8(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedRGBAlpha16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedGrayscaleAlpha8(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedGrayscaleAlpha16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsePngLayerIDAT.EncodeInterlacedAdam7(Stream: TStream;
  var ZLIBStream: TZStreamRec2);
var
...
function TsePngLayerIDAT.FilterToEncode: Byte;
var
...
begin
  ...
  for ii := FILTER_NONE TO FILTER_PAETH do
    if TsePngFilter(ii) in Owner.Filters then
    begin
      Run := 0;
      if Owner.Filters = [TsePngFilter(ii)] then
      begin
        Result := ii;
        exit;
      end;
...
function TsePngLayerPLTE.GetPaletteItem(Index: Byte): TRGBQuad;
begin
  if Index > Count - 1 then
    begin
    end
...
function TsePngLayerPLTE.LoadFromStream(Stream: TStream;
  const PngLayerName: TsePngLayerName; Size: Integer): Boolean;
type
  pPalEntry = ^PalEntry;
  PalEntry = record
...
function TsePngLayerPLTE.SaveToStream(Stream: TStream): Boolean;
var
  J: Integer;
  DataPtr: pByte;
  BitmapInfo: TMAXBITMAPINFO;
...
procedure TsePngLayerPLTE.Assign(Source: TsePngLayer);
begin
  if Source is TsePngLayerPLTE then
    fCount := TsePngLayerPLTE(Source).fCount;
end;

{TsePngLayergAMA}

procedure TsePngLayergAMA.Assign(Source: TsePngLayer);
begin
  if Source is TsePngLayergAMA then
    Gamma := TsePngLayergAMA(Source).Gamma;
end;

constructor TsePngLayergAMA.Create(Owner: TsePngImage);
begin
  inherited Create(Owner);
  Gamma := 1;
end;

function TsePngLayergAMA.GetValue: Cardinal;
begin
  if DataSize <> 4 then
  begin
    ResizeData(4);
...
function TsePngLayergAMA.LoadFromStream(Stream: TStream;
  const PngLayerName: TsePngLayerName; Size: Integer): Boolean;
var
  i: Integer;
  Value: Cardinal;
...
procedure TsePngLayergAMA.SetValue(const Value: Cardinal);
begin
...
procedure TsePngImage.Assign(Source: TPersistent);
begin
  if Source = nil then
    ClearPngLayers
  else if Source is TsePngImage then
    AssignPNG(Source as TsePngImage)
...
procedure TsePngImage.CreateAlpha;
var
  TRNS: TsePngLayerTRNS;
begin
  with Header do
    case ColorType of
      ...
      COLOR_PALETTE:
      begin
        if PngLayers.ItemFromClass(TsePngLayerTRNS) = nil then
          TRNS := PngLayers.Add(TsePngLayerTRNS) as TsePngLayerTRNS
        else
          TRNS := PngLayers.ItemFromClass(TsePngLayerTRNS) as TsePngLayerTRNS;
          with TRNS do
          begin
            ResizeData(256);
            Fillchar(PaletteValues[0], 256, 255);
...
procedure TsePngImage.ClearPngLayers;
var
  i: Integer;
begin
  InitializeGamma();
  for i := 0 TO Integer(PngLayers.Count) - 1 do
    TsePngLayer(PngLayers.Item[i]).Free;
  PngLayers.Count := 0;
end;

constructor TsePngImage.CreateBlank(ColorType, BitDepth: Cardinal;
  cx, cy: Integer);
var NewIHDR: TsePngLayerIHDR;
begin
  Create;
  ...
  InitializeGamma;
  BeingCreated := True;
  PngLayers.Add(TsePngLayerIEND);
  NewIHDR := PngLayers.Add(TsePngLayerIHDR) as TsePngLayerIHDR;
  ...
  if NewIHDR.HasPalette then
    TsePngLayerPLTE(PngLayers.Add(TsePngLayerPLTE)).fCount := 1 shl BitDepth;
  PngLayers.Add(TsePngLayerIDAT);
  BeingCreated := False;
end;

constructor TsePngImage.Create;
begin
  inherited Create;
  ...
  fPngLayerList := TsePngList.Create(Self);

end;

destructor TsePngImage.Destroy;
begin
  if fCanvas <> nil
  then
    begin
...
procedure TsePngImage.GetPixelInfo(var LineSize, Offset: Cardinal);
begin
...
function TsePngImage.GetHeight: Integer;
begin
  if HeaderPresent then
    Result := TsePngLayerIHDR(PngLayers.Item[0]).Height
  else Result := 0;
end;

function TsePngImage.GetWidth: Integer;
begin
  if HeaderPresent then
    Result := Header.Width
  else Result := 0;
...
function TsePngImage.GetEmpty: Boolean;
begin
  Result := (PngLayers.Count = 0);
end;

procedure TsePngImage.SetMaxIdatSize(const Value: Integer);
begin
  if Value < High(Word) then
    fMaxIdatSize := High(Word) else fMaxIdatSize := Value;
end;

function TsePngImage.GetHeader: TsePngLayerIHDR;
begin
  if (PngLayers.Count <> 0) and (PngLayers.Item[0] is TsePngLayerIHDR) then
    Result := PngLayers.Item[0] as TsePngLayerIHDR
  else
  begin
    Result := nil;
  end;
...
procedure TsePngImage.DrawPngLayerialTrans(DC: HDC; Rect: TRect);
...
var
  ...
  TransparencyPngLayer: TsePngLayertRNS;
  PalettePngLayer: TsePngLayerPLTE;
...
begin
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
    ...
    begin
      TransparencyPngLayer := TsePngLayertRNS(PngLayers.ItemFromClass(TsePngLayertRNS));
      PalettePngLayer := TsePngLayerPLTE(PngLayers.ItemFromClass(TsePngLayerPLTE));
...
procedure TsePngImage.Draw(ACanvas: TCanvas; const Rect: TRect);
var
  Header: TsePngLayerIHDR;
begin
  if Empty then Exit;
  Header := PngLayers.GetItem(0) as TsePngLayerIHDR;
...
procedure TsePngImage.LoadFromStream(Stream: TStream);
var
  Header    : Array[0..7] of Char;
  HasIDAT   : Boolean;
  PngLayerCount : Cardinal;
  PngLayerLength: Cardinal;
  PngLayerName  : TsePngLayerName;
begin
  PngLayerCount := 0;
  ClearPngLayers();
  ...
  repeat
    ...
    try if not TsePngLayer(PngLayers.Item[PngLayerCount - 1]).LoadFromStream(Stream,
...
procedure TsePngImage.SetHeight(Value: Integer);
begin
  Resize(Width, Value)
end;

procedure TsePngImage.SetWidth(Value: Integer);
begin
  Resize(Value, Height)
end;

function TsePngImage.GetTransparent: Boolean;
begin
  Result := (TransparencyMode <> bsptmNone);
end;

procedure TsePngImage.SaveToStream(Stream: TStream);
var
  j: Integer;
begin
...
procedure BuildHeader(Header: TsePngLayerIHDR; Handle: HBitmap; Info: pBitmap);
var
  DC: HDC;
begin
...
procedure TsePngImage.LoadFromResourceName(Instance: HInst;
  const Name: String);
var
...
procedure TsePngImage.LoadFromResourceID(Instance: HInst; ResID: Integer);
begin
  LoadFromResourceName(Instance, String(ResID));
end;

procedure TsePngImage.AssignTo(Dest: TPersistent);
...
var
  TRNS: TsePngLayerTRNS;
begin
  if Dest is TsePngImage then
    TsePngImage(Dest).AssignPNG(Self)
...
procedure TsePngImage.AssignHandle(Handle: HBitmap; Transparent: Boolean;
  TransparentColor: ColorRef);
var
...
begin
  ...
  ClearPngLayers();
  Header := TsePngLayerIHDR.Create(Self);
  BuildHeader(Header, Handle, @BitmapInfo);
  if Header.HasPalette then PLTE := TsePngLayerPLTE.Create(Self) else PLTE := nil;
  if Transparent then TRNS := TsePngLayerTRNS.Create(Self) else TRNS := nil;
  IDAT := TsePngLayerIDAT.Create(Self);
  IEND := TsePngLayerIEND.Create(Self);
  TsePngPointerList(PngLayers).Add(Header);
  if Header.HasPalette then TsePngPointerList(PngLayers).Add(PLTE);
  if Transparent then TsePngPointerList(PngLayers).Add(TRNS);
  TsePngPointerList(PngLayers).Add(IDAT);
  TsePngPointerList(PngLayers).Add(IEND);
...
procedure TsePngImage.AssignPNG(Source: TsePngImage);
var
  J: Integer;
begin
  ...
  FOR J := 0 TO PngLayers.Count - 1 DO
    ...
    begin
      PngLayers.SetItem(J, TsePngLayerClass(TsePngLayer(Item[J]).ClassType).Create(Self));
      TsePngLayer(PngLayers.Item[J]).Assign(TsePngLayer(Item[J]));
    end;
end;

function TsePngImage.GetAlphaScanline(const LineIndex: Integer): pByteArray;
begin
  with Header do
    if (ColorType = COLOR_RGBALPHA) or (ColorType = COLOR_GRAYSCALEALPHA) then
...
function TsePngImage.GetExtraScanline(const LineIndex: Integer): Pointer;
begin
  with Header do
...
function TsePngImage.GetScanline(const LineIndex: Integer): Pointer;
begin
  with Header do
...
procedure TsePngImage.InitializeGamma;
var
  i: Integer;
begin
  FOR i := 0 to 255 do
...
function TsePngImage.GetTransparencyMode: TsePngTransparencyMode;
var
  TRNS: TsePngLayerTRNS;
begin
  with Header do
...
procedure TsePngImage.RemoveTransparency;
var
  TRNS: TsePngLayerTRNS;
begin
  with Header do
...
function TsePngImage.GetTransparentColor: TColor;
var
  TRNS: TsePngLayerTRNS;
begin
  TRNS := PngLayers.ItemFromClass(TsePngLayerTRNS) as TsePngLayerTRNS;
...
procedure TsePngImage.SetTransparentColor(const Value: TColor);
var
  TRNS: TsePngLayerTRNS;
begin
  if HeaderPresent then
    ...
    COLOR_PALETTE, COLOR_RGB, COLOR_GRAYSCALE:
      begin
        TRNS := PngLayers.ItemFromClass(TsePngLayerTRNS) as TsePngLayerTRNS;
        if not Assigned(TRNS) then TRNS := PngLayers.Add(TsePngLayerTRNS) as TsePngLayerTRNS;
        TRNS.TransparentColor := ColorToRGB(Value);
      end;
    end;
end;

function TsePngImage.HeaderPresent: Boolean;
begin
  Result := ((PngLayers.Count <> 0) and (PngLayers.Item[0] is TsePngLayerIHDR));
end;

function GetByteArrayPixel(const png: TsePngImage; const X, Y: Integer): TColor;
var
  ByteData: Byte;
  DataDepth: Byte;
begin
  ...
  begin
    ...
    case ColorType of
      COLOR_PALETTE:
        with TsePngLayerPLTE(png.PngLayers.ItemFromClass(TsePngLayerPLTE)).Item[ByteData] do
...
procedure SetByteArrayPixel(const png: TsePngImage; const X, Y: Integer;
  const Value: TColor);
const
...
function GetRGBLinePixel(const png: TsePngImage;
...
procedure SetRGBLinePixel(const png: TsePngImage;
...
function GetGrayLinePixel(const png: TsePngImage;
  const X, Y: Integer): TColor;
var
  B: Byte;
begin
...
procedure SetGrayLinePixel(const png: TsePngImage;
 const X, Y: Integer; Value: TColor);
begin
  PByteArray(png.Scanline[Y])^[X] := GetRValue(Value);
end;

procedure TsePngImage.Resize(const CX, CY: Integer);
...
procedure TsePngImage.SetPixels(const X, Y: Integer; const Value: TColor);
begin
...
function TsePngImage.GetPixels(const X, Y: Integer): TColor;
begin
  if ((X >= 0) and (X <= Width - 1)) and
        ((Y >= 0) and (Y <= Height - 1)) then
    with Header do
...
function TsePngImage.GetPalette: HPALETTE;
begin
  Result := Header.ImagePalette;
end;

procedure TsePngLayerpHYs.Assign(Source: TsePngLayer);
begin
  fPPUnitY := TsePngLayerpHYs(Source).fPPUnitY;
  fPPUnitX := TsePngLayerpHYs(Source).fPPUnitX;
  fUnit := TsePngLayerpHYs(Source).fUnit;
end;

function TsePngLayerpHYs.LoadFromStream(Stream: TStream; const PngLayerName: TsePngLayerName;
  Size: Integer): Boolean;
begin
  Result := inherited LoadFromStream(Stream, PngLayerName, Size);
...
function TsePngLayerpHYs.SaveToStream(Stream: TStream): Boolean;
begin
  ResizeData(9);
...
procedure TsePngImage.DoSetPalette(Value: HPALETTE; const UpdateColors: boolean);
begin
  if (Header.HasPalette)  then
  begin
...
procedure TsePngImage.SetPalette(Value: HPALETTE);
begin
...
initialization
  ...
  {$IFDEF RegisterPNG}
  TPicture.RegisterFileFormat('PNG', 'PNG for SmartEffects VCL', TsePngImage);
  {$ENDIF}
finalization
  {$IFDEF RegisterPNG}
  TPicture.UnregisterGraphicClass(TsePngImage);
  {$ENDIF}
