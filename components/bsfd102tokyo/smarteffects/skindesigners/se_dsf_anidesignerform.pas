...
{       Version 2.97                                                }
{                                                                   }
{       Copyright (c) 2007-2015 Almediadev                          }
{       ALL RIGHTS RESERVED                                         }
{                                                                   }
...
uses
  ...
  DynamicSkinForm, SkinData, SkinCtrls, SkinBoxCtrls, spSkinShellCtrls;
...
{ TseSkinAnimationDesignerForm - designer form }
  ...
  private
    FAniCombo: TspSkinComboBox;
    FPreviewButton: TspSkinButton;
    FPreviewBox: TspSkinGroupBox;
    FSettingBox: TspSkinGroupBox;
    FDirectionCombo: TspSkinComboBox;
    FTimeEdit: TspSkinComboBox;
    FResEdit: TspSkinComboBox;
    FTileEdit: TspSkinComboBox;
    //
    FPreviewPaint: TPaintBox;
    FBitmap: TImage;
    FBitmapDialog: TspSkinOpenPictureDialog;
    FUpdating: boolean;
  ...
  public
    {$IFDEF DSF_SKIN}
    constructor CreateSkinDesigner(ASkinData: TspSkinData); virtual;
    {$ENDIF}
    destructor Destroy; override;
  end;

procedure ShowDSFSkinDesignerForm(const AAnimationItem: TseAnimation; ASkinData: TspSkinData);
...
begin
  ...
  FAniCombo := TspSkinComboBox.Create(Self);
  with FAniCombo do
  begin
    Parent := Self;
    Visible := true;
  ...
  FPreviewBox := TspSkinGroupBox.Create(Self);
  with FPreviewBox do
  begin
    Parent := Self;
    Visible := true;
    ...
    FPreviewButton := TspSkinButton.Create(Self);
  ...
  FSettingBox := TspSkinGroupBox.Create(Self);
  with FSettingBox do
  begin
    Parent := Self;
    Visible := true;
    ...
    FDirectionCombo := TspSkinComboBox.Create(Self);
    ...
    FTimeEdit := TspSkinComboBox.Create(Self);
    with FTimeEdit do
    begin
      Parent := FSettingBox;
      Visible := true;
    ...
    FResEdit := TspSkinComboBox.Create(Self);
    with FResEdit do
    begin
      Parent := FSettingBox;
      Visible := true;
    ...
    FTileEdit := TspSkinComboBox.Create(Self);
    with FTileEdit do
    begin
      Parent := FSettingBox;
      Visible := true;
  ...
  FBitmapDialog := TspSkinOpenPictureDialog.Create(Self);
  ...
  DSF.ClientWidth := Self.ClientWidth;
  DSF.ClientHeight := Self.ClientHeight;
  DSF.SkinData := ASkinData;
  DSF.BorderIcons := [];
  DSF.Sizeable := False;
end;
