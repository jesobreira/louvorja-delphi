...
uses
  ...
  Windows, CommCtrl, SyncObjs, Classes, Forms, SysUtils;
  {$ELSE}
  Windows, CommCtrl, SyncObjs, Classes, Forms, SysUtils, Multimon;
  {$ENDIF}
...
function UseThemesSlow: Boolean;
...
var
  ...
  DwmSetWindowAttribute: function(hwnd: HWND; dwAttribute: DWMWINDOWATTRIBUTE; pvAttribute: Pointer; size: cardinal): HRESULT; stdcall;
...
const
  ...
  CEMAGIC  = $5A4D;  // old EXE magic id: ð'MZ'
  CPEMAGIC = $4550;  // NT portable executable
begin
  result := nil;
  try
    if (not IsBadReadPtr(pointer(module), 2)) and (word(pointer(module)^) = CEMAGIC) then
    begin
...
begin
  ...
  p1 := nil;
  p2 := nil;
  while VirtualQuery(p1, mbi, sizeOf(mbi)) = sizeOf(mbi) do
  begin
    if (mbi.State = MEM_COMMIT) and
       (mbi.AllocationBase <> p2) and (mbi.AllocationBase = mbi.BaseAddress) and
       (GetModuleFileName(dword(mbi.AllocationBase), arrCh, MAX_PATH) > 0) then
    begin
...
begin
  ...
  if pinh <> nil then
  begin
    with pinh^.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT] do
    begin
      pid := pointer(module + VirtualAddress);
      eis := VirtualAddress + Size;
    end;
    if pid <> nil then
      while pid^.Name_ <> 0 do
      begin
...
begin
  for i1 := 0 to high(modules) do
    if IsWinNT or (modules[i1].handle < $80000000) then
...
begin
  ...
  begin
    ...
    begin
      ...
      if VirtualProtect(p1, 4, PAGE_EXECUTE_READWRITE, @c1) then
      begin
        pointer(p1^) := callback;
        VirtualProtect(p1, 4, c1, @c1);
      end;
...
procedure HookAPICall(Dll, Name: string; OrgAPICall, NewAPICall: Pointer;
  var SaveAPICall: Pointer);
begin
  PatchAPI(Dll, Name, OrgAPICall, SaveAPICall, NewAPICall)
end;
...
begin
  ...
  begin
    UnpatchAPI(OrgAPICall, SaveAPICall, NewAPICall);
    SaveAPICall := nil;
  end;
end;
...
var
  hDWMDLL: Cardinal = 0;
  RunOnWinVista: boolean = false;

function IsWinVista: boolean;
begin
  Result := RunOnWinVista;
end;

procedure SetupWinVista;
begin
...
var
  RunOnWinXP: boolean = false;
  ThemeLibrary: THandle = 0;
...
finalization
  if User32Handle > 0 then FreeLibrary(User32Handle);

  MonitorFromRectFunc := nil;
  MonitorFromPointFunc := nil;
